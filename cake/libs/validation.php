...
//         'hostname' => '(?:[a-z0-9][-a-z0-9]*\.)*(?:[a-z0-9][-a-z0-9]{0,62})\.(?:(?:[a-z]{2}\.)?[a-z]{2,4}|museum|travel)',
        // @see https://github.com/deregenboog/ecd/issues/399
        'hostname' => '(?:[a-z0-9][-a-z0-9]*\.)*(?:[a-z0-9][-a-z0-9]{0,62})\.(?:(?:[a-z]{2}\.)?[a-z]{2,}|museum|travel)',
    ];
    ...
    public function notEmpty($check)
    {
        ...
        if (empty($_this->check) && '0' != $_this->check) {
            return false;
        }
        $_this->regex = '/[^\s]+/m';
    ...
    public function cc($check, $type = 'fast', $deep = false, $regex = null)
    {
        ...
        $_this->check = str_replace(['-', ' '], '', $_this->check);
    ...
    public function email($check, $deep = false, $regex = null)
    {
        ...
        if (false === $_this->deep || null === $_this->deep) {
            return $return;
        }

        if (true === $return && preg_match('/@('.$_this->__pattern['hostname'].')$/i', $_this->check, $regs)) {
    ...
    public function ip($check, $type = 'both')
    {
        ...
        if ('ipv4' === $type || 'both' === $type) {
            $success |= $_this->_ipv4($check);
        }
        if ('ipv6' === $type || 'both' === $type) {
     ...
     * @return bool Success
     */
    public function _ipv4($check)
    {
        if (function_exists('filter_var')) {
            return false !== filter_var($check, FILTER_VALIDATE_IP, ['flags' => FILTER_FLAG_IPV4]);
        }
        $this->__populateIp();
        $this->check = $check;
     ...
     * @return bool Success
     */
    public function _ipv6($check)
    {
        if (function_exists('filter_var')) {
            return false !== filter_var($check, FILTER_VALIDATE_IP, ['flags' => FILTER_FLAG_IPV6]);
        }
        $this->__populateIp();
        $this->check = $check;
     ...
     * @return bool Success
     */
    public function multiple($check, $options = [])
    {
     ...
     **/
    public function _pass($method, $check, $classPrefix)
    {
