 ...
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/RSS.html#RSS
 */
class RssHelper extends XmlHelper
{
    ...
    public function channel($attrib = [], $elements = [], $content = null)
    {
        ...
        return $this->elem('channel', $attrib, $elems.$content, !(null === $content));
    }
    ...
    public function item($att = [], $elements = [])
    {
            ...
            switch ($key) {
                ...
                case 'enclosure':
                    if (is_string($val['url']) && is_file(WWW_ROOT.$val['url']) && file_exists(WWW_ROOT.$val['url'])) {
                        if (!isset($val['length']) && false === strpos($val['url'], '://')) {
        ...
        return $this->elem('item', $att, $content, !(null === $content));
    }
