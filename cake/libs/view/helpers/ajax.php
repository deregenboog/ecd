 ...
 * Helps doing AJAX using the Prototype library.
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#AJAX
 */
class AjaxHelper extends AppHelper
{
     ...
     * @var array
     */
    public $helpers = ['Html', 'Javascript', 'Form'];
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#link
     */
    public function link($title, $url = null, $options = [], $confirm = null)
    {
        if (!isset($url)) {
            $url = $title;
        }
        ...
        $htmlOptions = $this->__getHtmlOptions($options, ['url']);
        $options += ['safe' => true];
        ...
        $htmlDefaults = ['id' => 'link'.intval(mt_rand()), 'onclick' => ''];
     ...
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#remoteFunction
     */
     ...
     *
     * @see AjaxHelper::link()
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#remoteTimer
     */
    public function remoteTimer($options = null)
    {
     ...
     *
     * @see AjaxHelper::link()
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#form
     */
    public function form($params = null, $type = 'post', $options = [])
    {
        ...
        $htmlOptions = $this->__getHtmlOptions($options, ['model', 'with']);
        $htmlOptions = array_merge($htmlDefaults, $htmlOptions);

        $defaults = ['model' => $model, 'with' => "Form.serialize('{$htmlOptions['id']}')"];
     ...
     *
     * @see AjaxHelper::form()
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#submit
     */
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#observeField
     */
    public function observeField($field, $options = [])
    {
        ...
        $observer = 'Observer';
        if (!isset($options['frequency']) || 0 == intval($options['frequency'])) {
     ...
     * @return string ajax script
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#observeForm
     */
    public function observeForm($form, $options = [])
    {
        if (!isset($options['with'])) {
     ...
     * @return string Ajax script
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#autoComplete
     */
    public function autoComplete($field, $url = '', $options = [])
    {
        $var = '';
        ...
        $options = $this->_optionsToString($options, ['paramName', 'indicator']);
    ...
    public function div($id, $options = [])
    {
        ...
        $attr = $this->_parseAttributes(array_merge($options, ['id' => $id]));
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#isAjax
     */
    public function isAjax()
    {
        return isset($this->params['isAjax']) && true === $this->params['isAjax'];
    }
     ...
     * @return unknown
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#drag-drop
     */
    public function drag($id, $options = [])
    {
        ...
        $options = $this->_buildOptions(
            $this->_optionsToString($options, ['handle', 'constraint']), $this->dragOptions
        );
     ...
     */
    public function dropRemote($id, $options = [], $ajaxOptions = [])
    {
     ...
     *
     * @see          http://github.com/madrobby/scriptaculous/wikis/slider
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#slider
     */
    public function slider($id, $trackId, $options = [])
    {
        if (isset($options['var'])) {
            $var = 'var '.$options['var'].' = ';
            unset($options['var']);
     ...
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#editor
     */
     ...
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/AJAX.html#sortable
     */
    public function sortable($id, $options = [])
    {
        if (!empty($options['url'])) {
        ...
        $scrollIsObject = (
            ...
            'window' != $options['scroll'] &&
            0 !== strpos($options['scroll'], '$(')
        );
     ...
     */
    public function __optionsForAjax($options)
    {
        ...
        $jsOptions = array_merge($jsOptions, array_intersect_key($options, array_flip([
        ...
        foreach ($options as $key => $value) {
            switch ($key) {
                case 'type':
                    $jsOptions['asynchronous'] = ('synchronous' == $value) ? 'false' : 'true';
                break;
                case 'evalScripts':
     ...
     */
    public function __getHtmlOptions($options, $extra = [])
    {
        ...
        if (is_array($options)) {
            ...
            foreach ($options as $k => $v) {
                if (in_array($k, $acceptable)) {
                    ...
                    $out[] = "$k:$v";
                } elseif ('with' === $k && in_array('parameters', $acceptable)) {
                    $out[] = "parameters:${v}";
                }
            }
     ...
     */
    public function _buildObserver($klass, $name, $options = null)
    {
        ...
        $callback = $this->remoteFunction($options);
        $hasFrequency = !(!isset($options['frequency']) || 0 == intval($options['frequency']));
            ...
            if (isset($options[$callback])) {
                ...
                if (isset($options['bind'])) {
                    ...
                    $hasBinding = (
                        ...
                        (is_string($bind) && false !== strpos($bind, $callback))
                    );
     ...
     * @return array
     */
    public function _optionsToString($options, $stringOpts = [])
    {
        foreach ($stringOpts as $option) {
            ...
            if ($hasOption) {
                if (true === $options[$option] || 'true' === $options[$option]) {
                    $options[$option] = 'true';
                } elseif (false === $options[$option] || 'false' === $options[$option]) {
    ...
    {
        if (null != env('HTTP_X_UPDATE') && !empty($this->__ajaxBuffer)) {
            @ob_end_clean();
            ...
            if (1 == count($divs) && in_array($divs[0], $keys)) {
