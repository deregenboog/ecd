 ...
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/Time.html#Time
 */
class TimeHelper extends AppHelper
{
    ...
    public function convertSpecifiers($format, $time = null)
    {
        ...
        return preg_replace_callback('/\%(\w+)/', [$this, '__translateSpecifier'], $format);
    }
        ...
        switch ($specifier[1]) {
            ...
            case 'P':
                ...
                if (is_array($format)) {
                    ...
                    return ('P' == $specifier[1]) ? strtolower($meridiem) : strtoupper($meridiem);
                }
                break;
            case 'r':
                $complete = __c('t_fmt_ampm', 5, true);
                if ('t_fmt_ampm' != $complete) {
                    return str_replace('%p', $this->__translateSpecifier(['%p', 'p']), $complete);
                }
                break;
            case 'R':
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/Time.html#Formatting
     */
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/Time.html#Testing-Time
     */
    ...
    public function toQuarter($dateString, $range = false)
    {
        ...
        if (false !== $range) {
            ...
            switch ($date) {
                case 1:
                    $date = [$year.'-01-01', $year.'-03-31'];
                    break;
                case 2:
                    $date = [$year.'-04-01', $year.'-06-30'];
                    break;
                case 3:
                    $date = [$year.'-07-01', $year.'-09-30'];
                    break;
                case 4:
                    $date = [$year.'-10-01', $year.'-12-31'];
                    break;
            }
        }
    ...
    public function timeAgoInWords($dateTime, $options = [])
    {
        ...
        if ($diff >= 604800) {
            ...
            if (0 == $months && $years >= 1 && $diff < ($years * 31536000)) {
                $months = 11;
                --$years;
            }
     ...
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Helpers/Time.html#Formatting
     */
    public function relativeTime($dateTime, $options = [])
    {
    ...
    public function format($format, $date = null, $invalid = false, $userOffset = null)
    {
        ...
        if (is_numeric($_time) && false === $time) {
            $format = $date;

            return $this->i18nFormat($_time, $format, $invalid, $userOffset);
        }
        if (false === $time && false !== $invalid) {
            return $invalid;
        }
        if (false === $time) {
            return '';
