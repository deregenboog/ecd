...
class MediaView extends View
{
    ...
    public function render()
    {
        ...
        if (isset($extension) && isset($this->mimeType[strtolower($extension)]) && 0 == connection_status()) {
            $chunkSize = 8192;
            $buffer = '';
     ...
     * @param bool  $boolean
     */
    public function _header($header, $boolean = true)
    {
        if (is_array($header)) {
            foreach ($header as $string => $boolean) {
                if (is_numeric($string)) {
                    $this->_headers[] = [$boolean => true];
                } else {
                    $this->_headers[] = [$string => $boolean];
                }
            }

            return;
        }
        $this->_headers[] = [$header => $boolean];
    ...
    {
        return 0 == connection_status() && !connection_aborted();
    }
