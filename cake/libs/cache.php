...
class Cache
{
     ...
     */
    public function _buildEngine($name)
    {
        $config = $this->__config[$name];

        list($plugin, $class) = pluginSplit($config['engine']);
        $cacheClass = $class.'Engine';
        if (!class_exists($cacheClass) && false === $this->__loadEngine($class, $plugin)) {
            return false;
        }
        $cacheClass = $class.'Engine';
        $this->_engines[$name] = new $cacheClass();
        if ($this->_engines[$name]->init($config)) {
            if ($this->_engines[$name]->settings['probability'] && 0 === time() % $this->_engines[$name]->settings['probability']) {
     ...
     */
    public function __loadEngine($name, $plugin = null)
    {
        if ($plugin) {
    ...
    public static function set($settings = [], $value = null)
    {
        ...
        if (true === $self->__reset) {
            if (empty($settings)) {
                $self->__reset = false;
                $settings = $self->__config[$name];
            } else {
                if (is_string($settings) && null !== $value) {
    ...
    public static function read($key, $config = null)
    {
        ...
        if (null !== $config && $config !== $self->__name) {
            $self->set();
        }
    ...
    {
        if ('cache' == Configure::read('Session.save') && function_exists('session_write_close')) {
            session_write_close();
        }
    }
}
...
class CacheEngine
{
    ...
    public function init($settings = [])
    {
        $this->settings = array_merge(
            ['prefix' => 'cake_', 'duration' => 3600, 'probability' => 100],
            $this->settings,
            $settings
        );
     ...
     *
     * @param string $key   Identifier for the data
     * @param int    $value How much to substract
     *
     * @return New incremented value, false otherwise
     */
