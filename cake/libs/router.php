...
class Router
{
     ...
     * @var bool
     */
    public $__parseExtensions = false;
     ...
     * @static
     */
    public function getNamedExpressions()
    {
     ...
     * @static
     */
    public function connect($route, $defaults = [], $options = [])
    {
        $self = &self::getInstance();
        ...
        if ('CakeRoute' !== $routeClass && !is_subclass_of($Route, 'CakeRoute')) {
     ...
     * @static
     */
    public function connectNamed($named, $options = [])
    {
        $self = &self::getInstance();
        ...
        if (true == $options['reset'] || false === $self->named['rules']) {
            $self->named['rules'] = [];
        }
     ...
     *
     * @param bool $connect set to true or false depending on whether you want or don't want default routes
     * @static
     */
    public function defaults($connect = true)
    {
        $self = &self::getInstance();
     ...
     * @static
     */
    public function mapResources($controller, $options = [])
    {
        $self = &self::getInstance();
        $options = array_merge(['prefix' => '/', 'id' => $self->__named['ID'].'|'.$self->__named['UUID']], $options);
        ...
        foreach ((array) $controller as $ctlName) {
            ...
            foreach ($self->__resourceMap as $params) {
                ...
                self::connect($url,
                    ['controller' => $urlName, 'action' => $action, '[method]' => $params['method']],
    ...
    public function parse($url)
    {
        ...
        for ($i = 0, $len = count($self->routes); $i < $len; ++$i) {
            ...
            if (false !== ($r = $route->parse($url))) {
                ...
                if (isset($out['_args_'])) {
                    $argOptions['context'] = ['action' => $out['action'], 'controller' => $out['controller']];
     ...
     */
    public function __parseExtension($url)
    {
        $ext = null;

        if ($this->__parseExtensions) {
            if (1 === preg_match('/\.[0-9a-zA-Z]*$/', $url, $match)) {
     ...
     */
    public function __connectDefaultRoutes()
    {
        if ($plugins = App::objects('plugin')) {
            ...
            $shortParams = ['routeClass' => 'PluginShortRoute', 'plugin' => $pluginPattern];
            ...
            $this->connect('/:plugin', ['action' => 'index'], $shortParams);
            $this->connect('/:plugin/:controller', ['action' => 'index'], $match);
            $this->connect('/:plugin/:controller/:action/*', [], $match);
        }
        ...
        $this->connect('/:controller', ['action' => 'index']);
        $this->connect('/:controller/:action/*');

        if (false === $this->named['rules']) {
     ...
     * @static
     */
    public function setRequestInfo($params)
    {
        $self = &self::getInstance();
        $defaults = ['plugin' => null, 'controller' => null, 'action' => null];
     ...
     * @static
     */
    public function getParams($current = false)
    {
        $self = &self::getInstance();
        if ($current) {
     ...
     * @static
     */
    public function getParam($name = 'controller', $current = false)
    {
     ...
     * @static
     */
    public function getPaths($current = false)
    {
        $self = &self::getInstance();
        if ($current) {
     ...
     * @static
     */
    public function promote($which = null)
    {
        $self = &self::getInstance();
        if (null === $which) {
    ...
    {
        $self = &self::getInstance();
        $defaults = $params = ['plugin' => null, 'controller' => null, 'action' => 'index'];
        ...
        if (is_array($url)) {
            if (isset($url['base']) && false === $url['base']) {
                $base = null;
                unset($url['base']);
            }
            if (isset($url['full_base']) && true === $url['full_base']) {
                $full = true;
                unset($url['full_base']);
            }
            if (isset($url['?'])) {
            ...
            foreach ($self->__prefixes as $prefix) {
                ...
                if (isset($url[$prefix]) && 0 === strpos($url['action'], $prefix.'_')) {
            ...
            if ((false !== (strpos($url, '://')) || (0 === strpos($url, 'javascript:')) || (0 === strpos($url, 'mailto:'))) || (!strncmp($url, '#', 1))) {
                return $url;
            }
            if (empty($url)) {
                if (!isset($path['here'])) {
        ...
        return $output.$extension.$self->queryString($q, [], $escape).$frag;
    }
     ...
     * @see Router::url()
     */
    public function _handleNoRoute($url)
    {
        $named = $args = [];
        $skip = array_merge(
        ...
        list($args, $named) = [Set::filter($args, true), Set::filter($named, true)];
        foreach ($this->__prefixes as $prefix) {
            if (!empty($url[$prefix])) {
        ...
        if (empty($named) && empty($args) && (!isset($url['action']) || 'index' === $url['action'])) {
            $url['action'] = null;
        }
     ...
     * @static
     */
    public function matchNamed($param, $val, $rule, $context = [])
    {
        if (true === $rule || false === $rule) {
            return $rule;
        }
        if (is_string($rule)) {
            $rule = ['match' => $rule];
     ...
     * @static
     */
    public function queryString($q, $extra = [], $escape = false)
    {
     ...
     * @static
     */
    public function getArgs($args, $options = [])
    {
        $self = &self::getInstance();
        $pass = $named = [];
        $args = explode('/', $args);
        ...
        if (isset($options['named'])) {
            $greedy = isset($options['greedy']) && true === $options['greedy'];
        ...
        foreach ($args as $param) {
            ...
            $separatorIsPresent = false !== strpos($param, $self->named['separator']);
...
class CakeRoute
{
     ...
     */
    public function CakeRoute($template, $defaults = [], $options = [])
    {
    ...
    public function _writeRoute()
    {
        ...
        foreach ($namedElements[1] as $i => $name) {
            ...
            if (isset($this->options[$name])) {
                $option = null;
                if ('plugin' !== $name && array_key_exists($name, $this->defaults)) {
                    $option = '?';
                }
                $slashParam = '/\\'.$namedElements[0][$i];
                if (false !== strpos($parsed, $slashParam)) {
    ...
    public function match($url)
    {
        ...
        if (!$this->_greedy && array_diff_key($diff, $keyNames) != []) {
            return false;
        }
        ...
        if (array_intersect_key($filteredDefaults, $diffUnfiltered) !== []) {
            return false;
        }
