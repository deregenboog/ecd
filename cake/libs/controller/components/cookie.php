 ...
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Components/Cookies.html
 */
class CookieComponent extends Object
{
     ...
     */
    public function __expire($expires = null)
    {
        $now = time();
        if (is_null($expires)) {
     ...
     */
    public function __write($name, $value)
    {
     ...
     */
    public function __decrypt($values)
    {
        $decrypted = [];
        $type = $this->__type;
     ...
     */
    public function __explode($string)
    {
        $first = substr($string, 0, 1);
        if (false !== $first && ('{' === $first || '[' === $first) && function_exists('json_decode')) {
            $ret = json_decode($string, true);

            return (null != $ret) ? $ret : $string;
        }
        $array = [];
