 ...
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Components/Request-Handling.html
 */
class RequestHandlerComponent extends Object
{
     ...
     */
    public $__typesInitialized = false;
    ...
    public function startup(&$controller)
    {
        ...
        if ($this->requestedWith('xml')) {
            ...
            if (1 == count($xml->children) && is_object($dataNode = $xml->child('data'))) {
    ...
    public function beforeRedirect(&$controller, $url, $status = null)
    {
        ...
        echo $this->requestAction($url, ['return', 'bare' => false]);
        $this->_stop();
    }
    ...
    {
        return 'XMLHttpRequest' === env('HTTP_X_REQUESTED_WITH');
    }
    ...
    {
        return 1 == preg_match('/^(Shockwave|Adobe) Flash/', env('HTTP_USER_AGENT'));
    }
    ...
    {
        return 'post' == strtolower(env('REQUEST_METHOD'));
    }
    ...
    {
        return 'put' == strtolower(env('REQUEST_METHOD'));
    }
    ...
    {
        return 'get' == strtolower(env('REQUEST_METHOD'));
    }
    ...
    {
        return 'delete' == strtolower(env('REQUEST_METHOD'));
    }
    ...
    {
        if (null != env('HTTP_X_PROTOTYPE_VERSION')) {
    ...
    {
        if (null != env('HTTP_HOST')) {
            $sessHost = env('HTTP_HOST');
        }

        if (null != env('HTTP_X_FORWARDED_HOST')) {
    ...
    {
        if (!$safe && null != env('HTTP_X_FORWARDED_FOR')) {
            $ipaddr = preg_replace('/(?:,.*)/', '', env('HTTP_X_FORWARDED_FOR'));
        } else {
            if (null != env('HTTP_CLIENT_IP')) {
        ...
        if (null != env('HTTP_CLIENTADDRESS')) {
    ...
    {
        $this->__initializeTypes();
        if (!array_key_exists($type, $this->__requestContent) && false === strpos($type, '/')) {
            return false;
        }
        $defaults = ['index' => 0, 'charset' => null, 'attachment' => false];
        $options = array_merge($defaults, $options);

        if (false === strpos($type, '/') && isset($this->__requestContent[$type])) {
            $cType = null;
    ...
    {
        if (null == $this->__responseTypeSet) {
            return null;
        }
     ...
     */
    public function __initializeTypes()
    {
