...
class String
{
    ...
    public function tokenize($data, $separator = ',', $leftBound = '(', $rightBound = ')')
    {
        ...
        while ($offset <= $length) {
            ...
            for ($i = 0; $i < 3; ++$i) {
                if (false !== $offsets[$i] && ($offsets[$i] < $tmpOffset || $tmpOffset == -1)) {
                    $tmpOffset = $offsets[$i];
                }
            }
            if ($tmpOffset !== -1) {
                $buffer .= substr($data, $offset, ($tmpOffset - $offset));
                if ($data[$tmpOffset] == $separator && 0 == $depth) {
                    $results[] = $buffer;
                    $buffer = '';
                } else {
     ...
     * @static
     */
    public function insert($str, $data, $options = [])
    {
        $defaults = [
        ...
        if (false !== strpos($str, '?') && is_numeric(key($data))) {
            $offset = 0;
     ...
     * @see String::insert()
     */
    public function cleanInsert($str, $options)
    {
        $clean = $options['clean'];
        if (!$clean) {
            return $str;
