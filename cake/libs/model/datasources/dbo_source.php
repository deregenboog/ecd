...
class DboSource extends DataSource
{
    ...
    {
        $defaults = ['stats' => true, 'log' => $this->fullDebug];
    ...
    public function query()
    {
        ...
        } elseif (count($args) > 1 && (0 === strpos(strtolower($args[0]), 'findby') || 0 === strpos(strtolower($args[0]), 'findallby'))) {
            $params = $args[1];

            if (0 === strpos(strtolower($args[0]), 'findby')) {
                $all = false;
    ...
    public function fetchVirtualField(&$result)
    {
        if (isset($result[0]) && is_array($result[0])) {
            foreach ($result[0] as $field => $value) {
                if (false === strpos($field, $this->virtualFieldSeparator)) {
                    continue;
                }
    ...
    public function read(&$model, $queryData = [], $recursive = null)
    {
        ...
        if (null === $recursive && isset($queryData['recursive'])) {
        ...
        if (true === $queryData['callbacks'] || 'after' === $queryData['callbacks']) {
        ...
        if ($model->recursive > -1) {
            foreach ($_associations as $type) {
                foreach ($model->{$type} as $assoc => $assocData) {
                    ...
                    } elseif ($model->recursive > 1 && ('belongsTo' == $type || 'hasOne' == $type)) {
                        $db = &$this;
                    }
     ...
     */
    public function __filterResults(&$results, &$model, $filtered = [])
    {
        $filtering = [];
        $count = count($results);
                ...
                for ($j = 0; $j < $count2; ++$j) {
                    ...
                    if ($model->alias != $className && !in_array($className, $filtered)) {
                        ...
                        if (isset($model->{$className}) && is_object($model->{$className})) {
                            $data = $model->{$className}->afterFind([[$className => $results[$i][$className]]], false);
                        }
    ...
    public function queryAssociation(&$model, &$linkModel, $type, $association, $assocData, &$queryData, $external = false, &$resultSet, $recursive, $stack)
    {
        if ($query = $this->generateAssociationQuery($model, $linkModel, $type, $association, $assocData, $queryData, $external, $resultSet)) {
            ...
            if ('hasMany' === $type && empty($assocData['limit']) && !empty($assocData['foreignKey'])) {
                $ins = $fetch = [];
                for ($i = 0; $i < $count; ++$i) {
            ...
            } elseif ('hasAndBelongsToMany' === $type) {
                $ins = $fetch = [];
                for ($i = 0; $i < $count; ++$i) {
                    if ($in = $this->insertQueryData('{$__cakeID__$}', $resultSet[$i], $association, $assocData, $model, $linkModel, $stack)) {
                        $ins[] = $in;
                ...
                $joinKeys = [$foreignKey, $model->hasAndBelongsToMany[$association]['associationForeignKey']];
                    ...
                    if ($recursive > 0) {
                        foreach ($linkModel->__associations as $type1) {
                            foreach ($linkModel->{$type1} as $assoc1 => $assocData1) {
                                ...
                                if (('belongsTo' === $type1) || ($deepModel->alias === $model->alias && 'belongsTo' === $type) || ($deepModel->alias != $model->alias)) {
                                    $tmpStack = $stack;
                                    $tmpStack[] = $assoc1;
                                    if ($linkModel->useDbConfig == $deepModel->useDbConfig) {
                                        $db = &$this;
     ...
     */
    public function __mergeHasMany(&$resultSet, $merge, $association, &$model, &$linkModel)
    {
     ...
     * @param bool         $selfJoin
     */
    public function __mergeAssociation(&$data, $merge, $association, $type, $selfJoin = false)
    {
            ...
            if (isset($merge[$association])) {
                ...
                if (!isset($data[$association])) {
                    ...
                    if (is_array($merge[0][$association])) {
                        ...
                        $data[$association] = array_merge($data[$association], [$association => []]);
                    }
                }
            }
        } else {
     ...
     * @return mixed
     */
    public function generateAssociationQuery(&$model, &$linkModel, $type, $association = null, $assocData = [], &$queryData, $external = false, &$resultSet)
    {
        $queryData = $this->__scrubQueryData($queryData);
        ...
        if ((!$external && in_array($type, ['hasOne', 'belongsTo']) && false === $this->__bypass) || $external) {
        ...
        switch ($type) {
            ...
            case 'belongsTo':
                ...
                if (!$self && $external) {
                    foreach ($conditions as $key => $condition) {
                        if (is_numeric($key) && false !== strpos($condition, $model->alias.'.')) {
                ...
                if ($external) {
                    ...
                    $query = array_merge(['order' => $assocData['order'], 'limit' => $assocData['limit']], $query);
                } else {
                    $join = [
                        'table' => $linkModel,
                        'alias' => $alias,
            ...
            case 'hasMany':
                ...
                if (!empty($assocData['foreignKey'])) {
                    $assocData['fields'] = array_merge($assocData['fields'], $this->fields($linkModel, $alias, ["{$alias}.{$assocData['foreignKey']}"]));
                }
                $query = [
    ...
    {
        $assoc = array_merge(['external' => false, 'self' => false], $assoc);
        ...
        switch (true) {
            case $assoc['external'] && 'hasOne' == $type:
                return ["{$alias}.{$assoc['foreignKey']}" => '{$__cakeID__$}'];
            break;
            case $assoc['external'] && 'belongsTo' == $type:
                return ["{$alias}.{$linkModel->primaryKey}" => '{$__cakeForeignKey__$}'];
            break;
            case !$assoc['external'] && 'hasOne' == $type:
                return ["{$alias}.{$assoc['foreignKey']}" => $this->identifier("{$model->alias}.{$model->primaryKey}")];
            break;
            case !$assoc['external'] && 'belongsTo' == $type:
                return ["{$model->alias}.{$assoc['foreignKey']}" => $this->identifier("{$alias}.{$linkModel->primaryKey}")];
            break;
            case 'hasMany' == $type:
                return ["{$alias}.{$assoc['foreignKey']}" => ['{$__cakeID__$}']];
            break;
            case 'hasAndBelongsToMany' == $type:
                return [
     ...
     * @return array
     */
    public function __mergeConditions($query, $assoc)
    {
        if (empty($assoc)) {
            return $query;
        }
     ...
     */
    public function _prepareUpdateFields(&$model, $fields, $quoteValues = true, $alias = false)
    {
        $quotedAlias = $this->startQuote.$model->alias.$this->endQuote;

        $updates = [];
        foreach ($fields as $field => $value) {
            if ($alias && false === strpos($field, '.')) {
                $quoted = $model->escapeField($field);
            } elseif (!$alias && false !== strpos($field, '.')) {
     ...
     * @return array
     */
    public function _getJoins($model)
    {
        $join = [];
    ...
    public function defaultConditions(&$model, $conditions, $useAlias = true)
    {
        ...
        $exists = $model->exists();
        if (!$exists && (null !== $conditions || !empty($model->__safeUpdateMode))) {
            return false;
        } elseif (!$exists) {
            return null;
        }
    ...
    {
        foreach (['conditions', 'fields', 'joins', 'order', 'limit', 'offset', 'group'] as $key) {
            if (empty($data[$key])) {
                $data[$key] = [];
            }
        }
     ...
     * @return array
     */
    public function _constructVirtualFields(&$model, $alias, $fields)
    {
        $virtual = [];
    ...
    public function fields(&$model, $alias = null, $fields = [], $quote = true)
    {
        ...
        if ($count >= 1 && !in_array($fields[0], ['*', 'COUNT(*)'])) {
            for ($i = 0; $i < $count; ++$i) {
                ...
                if (is_object($fields[$i]) && isset($fields[$i]->type) && 'expression' === $fields[$i]->type) {
    ...
    public function conditions($conditions, $quoteValues = true, $where = true, $model = null)
    {
        if (is_object($model)) {
            ...
            $cacheKey = [$conditions, $quoteValues, $where];
        }
        $cacheKey = md5(serialize($cacheKey));
        if ($return = $this->cacheMethod(__FUNCTION__, $cacheKey)) {
            return $return;
    ...
    public function conditionKeysToString($conditions, $quoteValues = true, $model = null)
    {
        ...
        foreach ($conditions as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                $valueInsert = (!empty($value) && (substr_count($key, '?') === $valueCount || (substr_count($key, ':') === $valueCount && 0 === preg_match("/(\:){".$valueCount.'}/', $key))));
            }
    ...
    public function __parseKey(&$model, $key, $value)
    {
        ...
        if (!strpos($key, ' ')) {
            ...
            if (!preg_match($operatorMatch, trim($operator)) && false !== strpos($operator, ' ')) {
                $key = $key.' '.$operator;
                $split = strrpos($key, ' ');
    ...
    public function __quoteFields($conditions)
    {
        ...
        $conditions = preg_replace_callback('/(?:[\'\"][^\'\"\\\]*(?:\\\.[^\'\"\\\]*)*[\'\"])|([a-z0-9_'.$start.$end.']*\\.[a-z0-9_'.$start.$end.']*)/i', [&$this, '__quoteMatchedField'], $conditions);
     ...
     */
    public function __quoteMatchedField($match)
    {
    ...
    public function order($keys, $direction = 'ASC', $model = null)
    {
        ...
        while (!empty($keys)) {
            ...
            if (is_array($key)) {
                ...
                continue;
            } elseif (is_object($key) && isset($key->type) && 'expression' === $key->type) {
                $result[] = $key->value;
                continue;
            }
            ...
            if (strpos($key, '.')) {
                $key = preg_replace_callback('/([a-zA-Z0-9_-]{1,})\\.([a-zA-Z0-9_-]{1,})/', [&$this, '__quoteMatchedField'], $key);
            }
    ...
    public function createSchema($schema, $tableName = null)
    {
        ...
        foreach ($schema->tables as $curTable => $columns) {
            if (!$tableName || $tableName == $curTable) {
                $cols = $colList = $indexes = $tableParameters = [];
                $primary = null;
                ...
                foreach ($columns as $name => $col) {
                    ...
                    if (isset($col['key']) && 'primary' == $col['key']) {
                        $primary = $name;
                    }
                    if ('indexes' !== $name && 'tableParameters' !== $name) {
                        $col['name'] = $name;
                        if (!isset($col['type'])) {
                            $col['type'] = 'string';
                        }
    ...
    public function buildColumn($column)
    {
        ...
        if (('integer' == $column['type'] || 'float' == $column['type']) && isset($column['default']) && '' === $column['default']) {
            $column['default'] = null;
        }
        ...
        } elseif (isset($column['key']) && 'primary' == $column['key']) {
            $out .= ' NOT NULL';
        } elseif (isset($column['default']) && isset($column['null']) && false == $column['null']) {
            $out .= ' DEFAULT '.$this->value($column['default'], $type).' NOT NULL';
        } elseif (isset($column['default'])) {
            $out .= ' DEFAULT '.$this->value($column['default'], $type);
        } elseif ('timestamp' !== $type && !empty($column['null'])) {
            $out .= ' DEFAULT NULL';
        } elseif ('timestamp' === $type && !empty($column['null'])) {
            $out .= ' NULL';
        } elseif (isset($column['null']) && false == $column['null']) {
            $out .= ' NOT NULL';
        }
        if ('timestamp' == $type && isset($column['default']) && 'current_timestamp' == strtolower($column['default'])) {
