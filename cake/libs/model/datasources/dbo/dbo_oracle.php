...
class DboOracle extends DboSource
{
    ...
    public $columns = [
        ...
        'integer' => ['name' => 'number'],
        'float' => ['name' => 'float'],
        'datetime' => ['name' => 'date', 'format' => 'Y-m-d H:i:s'],
        'timestamp' => ['name' => 'date', 'format' => 'Y-m-d H:i:s'],
        'time' => ['name' => 'date', 'format' => 'Y-m-d H:i:s'],
        'date' => ['name' => 'date', 'format' => 'Y-m-d H:i:s'],
        'binary' => ['name' => 'bytea'],
    ...
    public function describe(&$model)
    {
        ...
        for ($i = 0; $row = $this->fetchRow(); ++$i) {
            $fields[strtolower($row[0]['COLUMN_NAME'])] = [
    ...
    public function constraint($action, $table)
    {
        ...
        if (empty($this->_keyConstraints)) {
            ...
            $this->execute($sql);
            while ($row = $this->fetchRow()) {
                $this->_keyConstraints[] = [$row[0]['table_name'], $row['c']['constraint_name']];
            }
        }
        ...
        foreach ($this->_constraints as $c) {
            if (in_array($c['r_constraint_name'], $relatedKeys)) {
                $constraints[] = [$c['table_name'], $c['constraint_name']];
            }
        }
    ...
    public function column($real)
    {
        ...
        if (in_array($col, ['date', 'timestamp'])) {
            return $col;
        }
        if (false !== strpos($col, 'number')) {
            return 'integer';
        }
        if (false !== strpos($col, 'integer')) {
            return 'integer';
        }
    ...
    public function queryAssociation(&$model, &$linkModel, $type, $association, $assocData, &$queryData, $external = false, &$resultSet, $recursive, $stack)
    {
        if ($query = $this->generateAssociationQuery($model, $linkModel, $type, $association, $assocData, $queryData, $external, $resultSet)) {
            ...
            if ('hasMany' === $type && (!isset($assocData['limit']) || empty($assocData['limit']))) {
                $ins = $fetch = [];
                for ($i = 0; $i < $count; ++$i) {
                        ...
                        foreach ($fetch as $j => $data) {
                            if (isset($data[$with]) && $data[$with][$foreignKey] === $row[$model->alias][$model->primaryKey]) {
                                if ($habtmFieldsCount > 2) {
                                    ...
                                    $merge[] = Set::diff($data, [$with => $data[$with]]);
                                }
                            }
                        }
