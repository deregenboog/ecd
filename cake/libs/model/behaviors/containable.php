 ...
 *
 * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Behaviors/Containable.html#Containable
 */
class ContainableBehavior extends ModelBehavior
{
    ...
    public function beforeFind(&$Model, $query)
    {
        ...
        if (
            $noContain || empty($contain) || (isset($contain[0]) && null === $contain[0])
        ) {
            if ($noContain) {
                $query['recursive'] = -1;
            }
                ...
                foreach ($Model->{$type} as $assoc => $data) {
                    if ($Model->useDbConfig == $Model->{$assoc}->useDbConfig && !empty($data['fields'])) {
                        foreach ((array) $data['fields'] as $field) {
                            $query['fields'][] = (false === strpos($field, '.') ? $assoc.'.' : '').$field;
                        }
                    }
                }
            }
            ...
            foreach ($mandatory[$Model->alias] as $field) {
                if ('--primaryKey--' == $field) {
                    ...
                    if ($Model->useDbConfig == $Model->{$modelName}->useDbConfig) {
                        $field = $modelName.'.'.(
                            ('--primaryKey--' === $field) ? $Model->$modelName->primaryKey : $field
                        );
                    } else {
                        $field = null;
                    }
     ...
     *
     * @see http://book.cakephp.org/1.3/en/The-Manual/Core-Behaviors/Containable.html#Containable#Using-Containable-1324
     */
     ...
     */
    public function containments(&$Model, $contain, $containments = [], $throwErrors = null)
    {
        $options = ['className', 'joinTable', 'with', 'foreignKey', 'associationForeignKey', 'conditions', 'fields', 'order', 'limit', 'offset', 'unique', 'finderQuery', 'deleteQuery', 'insertQuery'];
        $keep = [];
        $depth = [];
        if (null === $throwErrors) {
     ...
     * @return array Fields
     */
    public function fieldDependencies(&$Model, $map, $fields = [])
    {
        if (false === $fields) {
