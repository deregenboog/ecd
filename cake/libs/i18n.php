 ...
 * Included libraries.
 */
App::import('Core', ['l10n', 'Multibyte']);
...
class I18n extends Object
{
    ...
    public function translate($singular, $plural = null, $domain = null, $category = 6, $count = null)
    {
        ...
        if (false !== strpos($singular, "\r\n")) {
            $singular = str_replace("\r\n", "\n", $singular);
        }
        if (null !== $plural && false !== strpos($plural, "\r\n")) {
            $plural = str_replace("\r\n", "\n", $plural);
        }
        ...
        if ('LC_TIME' == $_this->category) {
            return $_this->__translateTime($singular, $domain);
        }

        if (!isset($count)) {
            $plurals = 0;
        } elseif (!empty($_this->__domains[$domain][$_this->__lang][$_this->category]['%plural-c']) && false === $_this->__noLocale) {
     ...
     */
    public function __pluralGuess($header, $n)
    {
        if (!is_string($header) || 'nplurals=1;plural=0;' === $header || !isset($header[0])) {
            return 0;
        }

        if ('nplurals=2;plural=n!=1;' === $header) {
            return 1 != $n ? 1 : 0;
        } elseif ('nplurals=2;plural=n>1;' === $header) {
            return $n > 1 ? 1 : 0;
        }
        ...
        if (strpos($header, 'plurals=3')) {
            if (strpos($header, '100!=11')) {
                ...
                return 1 == $n % 10 && 11 != $n % 100 ? 0 : (0 != $n ? 1 : 2);
            ...
            return 1 == $n % 10 ? 0 : (2 == $n % 10 ? 1 : 2);
        } elseif (strpos($header, 'plurals=4')) {
            if (strpos($header, '100==2')) {
                return 1 == $n % 100 ? 0 : (2 == $n % 100 ? 1 : (3 == $n % 100 || 4 == $n % 100 ? 2 : 3));
            } elseif (strpos($header, 'n>=3')) {
                return 1 == $n ? 0 : (2 == $n ? 1 : (0 == $n || ($n >= 3 && $n <= 10) ? 2 : 3));
            } elseif (strpos($header, '100>=1')) {
                return 1 == $n ? 0 : (0 == $n || ($n % 100 >= 1 && $n % 100 <= 10) ? 1 : ($n % 100 >= 11 && $n % 100 <= 20 ? 2 : 3));
            }
     ...
     */
    public function __bindTextDomain($domain)
    {
        ...
        if (empty($this->__domains[$domain][$this->__lang][$this->category])) {
            $this->__domains[$domain][$this->__lang][$this->category] = [];
     ...
     */
    public function __loadMo($file, $domain)
    {
        ...
        if ($data) {
            ...
            if (('950412de' == dechex($magic) || 'ffffffff950412de' == dechex($magic)) && 0 == $version) {
                for ($n = 0; $n < $count; ++$n) {
     ...
     */
    public function __loadPo($file, $domain)
    {
        $type = 0;
        $translations = [];
        $translationKey = '';
        ...
        do {
            ...
            if (preg_match('/msgid[[:space:]]+"(.+)"$/i', $line, $regs)) {
                ...
                $type = 2;
                $translationKey = '';
            } elseif (preg_match('/^"(.*)"$/i', $line, $regs) && (1 == $type || 2 == $type || 3 == $type)) {
                $type = 3;
                $translationKey .= stripcslashes($regs[1]);
            } elseif (preg_match('/msgstr[[:space:]]+"(.+)"$/i', $line, $regs) && (1 == $type || 3 == $type) && $translationKey) {
                $translations[$translationKey] = stripcslashes($regs[1]);
                $type = 4;
            } elseif (preg_match('/msgstr[[:space:]]+""$/i', $line, $regs) && (1 == $type || 3 == $type) && $translationKey) {
                $type = 4;
                $translations[$translationKey] = '';
            } elseif (preg_match('/^"(.*)"$/i', $line, $regs) && 4 == $type && $translationKey) {
                $translations[$translationKey] .= stripcslashes($regs[1]);
            } elseif (preg_match('/msgid_plural[[:space:]]+".*"$/i', $line, $regs)) {
                $type = 6;
            } elseif (preg_match('/^"(.*)"$/i', $line, $regs) && 6 == $type && $translationKey) {
                $type = 6;
            } elseif (preg_match("/msgstr\[(\d+)\][[:space:]]+\"(.+)\"$/i", $line, $regs) && (6 == $type || 7 == $type) && $translationKey) {
                $plural = $regs[1];
                ...
                $type = 7;
            } elseif (preg_match('/^"(.*)"$/i', $line, $regs) && 7 == $type && $translationKey) {
                $translations[$translationKey][$plural] .= stripcslashes($regs[1]);
            } elseif (preg_match('/msgstr[[:space:]]+"(.+)"$/i', $line, $regs) && 2 == $type && !$translationKey) {
                $header .= stripcslashes($regs[1]);
                $type = 5;
            } elseif (preg_match('/msgstr[[:space:]]+""$/i', $line, $regs) && !$translationKey) {
                $header = '';
                $type = 5;
            } elseif (preg_match('/^"(.*)"$/i', $line, $regs) && 5 == $type) {
                $header .= stripcslashes($regs[1]);
            } else {
     ...
     */
    public function __loadLocaleDefinition($file, $domain = null)
    {
        $comment = '#';
        $escape = '\\';
        $currentToken = false;
        ...
        while ($line = fgets($file)) {
            ...
            if ('comment_char' === $parts[0]) {
                $comment = $parts[1];
                continue;
            }
            if ('escape_char' === $parts[0]) {
                $escape = $parts[1];
                continue;
            }
            $count = count($parts);
            ...
            foreach ($value as $i => $val) {
                $val = trim($val, '"');
                $val = preg_replace_callback('/(?:<)?(.[^>]*)(?:>)?/', [&$this, '__parseLiteralValue'], $val);
     ...
     */
    public function __parseLiteralValue($string)
    {
        $string = $string[1];
     ...
     */
    public function __translateTime($format, $domain)
    {
