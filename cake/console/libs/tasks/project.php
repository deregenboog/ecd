...
class ProjectTask extends Shell
{
    ...
    public function execute($project = null)
    {
        ...
        if ($project) {
            $this->Dispatch->parseParams(['-app', $project]);
            $project = $this->params['working'];
        }

        if (empty($this->params['skel'])) {
            $this->params['skel'] = '';
            if (true === is_dir(CAKE.'console'.DS.'templates'.DS.'skel')) {
        ...
        if ($project) {
            $response = false;
            while (false == $response && true === is_dir($project) && file_exists($project.'config'.'core.php')) {
        ...
        if ($this->bake($project)) {
            ...
            if (true === $this->securitySalt($path)) {
            ...
            if (true === $this->securityCipherSeed($path)) {
    ...
    public function bake($path, $skel = null, $skip = ['empty'])
    {
        ...
        $looksGood = $this->in(__('Look okay?', true), ['y', 'n', 'q'], 'y');

        if ('y' == strtolower($looksGood)) {
            $verbose = $this->in(__('Do you want verbose output?', true), ['y', 'n'], 'n');
