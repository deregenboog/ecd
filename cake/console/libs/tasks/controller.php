...
class ControllerTask extends BakeTask
{
    ...
    public function execute()
    {
        ...
        if (isset($this->args[0])) {
            ...
            if (!empty($this->args[1]) && ('public' == $this->args[1] || 'scaffold' == $this->args[1])) {
                $this->out(__('Baking basic crud methods for ', true).$controller);
                $actions = $this->bakeActions($controller);
            } elseif (!empty($this->args[1]) && 'admin' == $this->args[1]) {
            ...
            if (!empty($this->args[2]) && 'admin' == $this->args[2]) {
                $admin = $this->Project->getPrefix();
                if ($admin) {
                    $this->out(sprintf(__('Adding %s methods', true), $admin));
    ...
    public function __interactive()
    {
        ...
        if ('y' == strtolower($doItInteractive)) {
            $this->interactive = true;
            $useDynamicScaffold = $this->in(
                __('Would you like to use dynamic scaffolding?', true), ['y', 'n'], 'n'
            );

            if ('y' == strtolower($useDynamicScaffold)) {
                $wannaBakeCrud = 'n';
                $actions = 'scaffold';
            } else {
                ...
                $wannaUseSession = $this->in(
                    __('Would you like to use Session flash messages?', true), ['y', 'n'], 'y'
                );
            }
        } else {
            list($wannaBakeCrud, $wannaBakeAdminCrud) = $this->_askAboutMethods();
        ...
        if ('y' == strtolower($wannaBakeCrud)) {
            $actions = $this->bakeActions($controllerName, null, 'y' == strtolower($wannaUseSession));
        }
        if ('y' == strtolower($wannaBakeAdminCrud)) {
            $admin = $this->Project->getPrefix();
            $actions .= $this->bakeActions($controllerName, $admin, 'y' == strtolower($wannaUseSession));
        }
     ...
     */
    public function _askAboutMethods()
    {
    ...
    {
        $isScaffold = ('scaffold' === $actions) ? true : false;
     ...
     */
    public function _doPropertyChoices($prompt, $example)
    {
     ...
     * @param bool   $interactive whether you are using listAll interactively and want options output
     *
