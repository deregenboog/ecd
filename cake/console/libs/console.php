...
class ConsoleShell extends Shell
{
    ...
    public function main($command = null)
    {
        while (true) {
            ...
            switch ($command) {
                ...
                break;
                case true == preg_match("/^(\w+) bind (\w+) (\w+)/", $command, $tmp):
                    foreach ($tmp as $data) {
                ...
                break;
                case true == preg_match("/^(\w+) unbind (\w+) (\w+)/", $command, $tmp):
                    foreach ($tmp as $data) {
                        $data = strip_tags($data);
                        $data = str_replace($this->badCommandChars, '', $data);
                    }
                ...
                break;
                case true == preg_match("/^(\w+) columns/", $command, $tmp):
                ...
                break;
                case true == preg_match("/^routes\s+reload/i", $command, $tmp):
                ...
                break;
                case true == preg_match("/^routes\s+show/i", $command, $tmp):
                    $router = &Router::getInstance();
                    $this->out(implode("\n", Set::extract($router->routes, '{n}.0')));
                break;
                case true == preg_match("/^route\s+(\(.*\))$/i", $command, $tmp):
     ...
     */
    public function _isValidModel($modelToCheck)
    {
