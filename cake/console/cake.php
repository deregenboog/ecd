...
class ShellDispatcher
{
     ...
     */
    public function ShellDispatcher($args = [])
    {
        set_time_limit(0);
        ...
        $this->__buildPaths();
        $this->_stop(false === $this->dispatch() ? 1 : 0);
    }

    /**
     * Defines core configuration.
     */
    public function __initConstants()
    {
    ...
    public function _initEnvironment()
    {
        ...
        if (basename(__FILE__) != basename($this->args[0])) {
            ...
            if ('n' == $this->getInput('Continue anyway?', ['y', 'n'], 'y')) {
                $this->_stop();
            }
        }
    ...
    public function dispatch()
    {
        ...
        if (is_a($Shell, 'Shell')) {
            ...
            if (in_array($task, $Shell->taskNames)) {
                ...
                if (isset($this->args[0]) && 'help' == $this->args[0]) {
    ...
    public function _getShell($plugin = null)
    {
        foreach ($this->shellPaths as $path) {
            ...
            if ((false !== strpos($path, $pluginShellPath) || !$plugin) && file_exists($this->shellPath)) {
                $loaded = true;
                break;
            }
        }
    ...
    public function getInput($prompt, $options = null, $default = null)
    {
        ...
        if (null !== $default && ('' === $result || null === $result)) {
            return $default;
        }
    ...
    public function parseParams($params)
    {
        ...
        $defaults = ['app' => 'app', 'root' => dirname(dirname(dirname(__FILE__))), 'working' => null, 'webroot' => 'webroot'];
        $params = array_merge($defaults, array_intersect_key($this->params, $defaults));
        $isWin = false;
        foreach ($defaults as $default => $value) {
            if (false !== strpos($params[$default], '\\')) {
                $isWin = true;
                break;
            }
        }
     ...
     * @return array params
     */
    public function __parseParams($params)
    {
        $count = count($params);
        for ($i = 0; $i < $count; ++$i) {
            if (isset($params[$i])) {
    ...
    public function help()
    {
        ...
        foreach ($this->shellPaths as $path) {
            ...
            if (preg_match('@plugins[\\\/]([^\\\/]*)@', $path, $matches)) {
                ...
                $type = $matches[1];
            } elseif (0 === strpos($path, CAKE_CORE_INCLUDE_PATH.DS.'cake')) {
                $type = 'CORE';
            } else {
                $type = 'app';
            }
