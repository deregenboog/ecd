...
class EmailTestComponent extends EmailComponent
{
     ...
     * @return mixed
     */
    public function smtpSend($data, $code = '250')
    {
     ...
     */
    public function _smtpSend($data, $code = '250')
    {
        if ($this->_debug) {
            $this->smtpSend .= $data."\n";
     ...
     */
    public function setConnectionSocket(&$socket)
    {
     ...
     * Wrapper for testing.
     */
    public function formatAddress($string, $smtp = false)
    {
...
class EmailComponentTest extends CakeTestCase
{
     ...
     * @return string
     */
    public function __osFix($string)
    {
        return str_replace(["\r\n", "\r"], "\n", $string);
    }
    ...
    public function testSmtpConfig()
    {
        ...
        $this->Controller->EmailTest->smtpOptions = ['port' => 80];
        $this->Controller->EmailTest->send('anything');
        $config['port'] = 80;
        $this->assertEqual($config, $this->Controller->EmailTest->smtpOptions);
    }
     ...
     */
    public function testBadSmtpSend()
    {
        if ($this->skipIf(!@fsockopen('localhost', 25), '%s No SMTP server running on localhost')) {
            return;
        }
...
TEMPDOC;
     ...
     * testSmtpEhlo method.
     */
    public function testSmtpEhlo()
    {
        if ($this->skipIf(!@fsockopen('localhost', 25), '%s No SMTP server running on localhost')) {
            return;
        }

        $connection = new CakeSocket(['protocol' => 'smtp', 'host' => 'localhost', 'port' => 25]);
        $this->Controller->EmailTest->setConnectionSocket($connection);
        $this->Controller->EmailTest->smtpOptions['timeout'] = 10;
        $this->assertTrue($connection->connect());
        $this->assertTrue(false !== $this->Controller->EmailTest->smtpSend(null, '220'));
        $this->skipIf(false === $this->Controller->EmailTest->smtpSend('EHLO locahost', '250'), '%s do not support EHLO.');
...
TEMPDOC;
     ...
     */
    public function testSmtpSendMultipleTo()
    {
        if ($this->skipIf(!@fsockopen('localhost', 25), '%s No SMTP server running on localhost')) {
            return;
        }
        $this->Controller->EmailTest->reset();
        $this->Controller->EmailTest->to = ['postmaster@localhost', 'root@localhost'];
        $this->Controller->EmailTest->from = 'noreply@example.com';
     ...
     */
    public function testSmtpSendHostWithPort()
    {
        $bkp = env('HTTP_HOST');
     ...
     */
    public function testAuthenticatedSmtpSend()
    {
        if ($this->skipIf(!@fsockopen('localhost', 25), '%s No SMTP server running on localhost')) {
            return;
        }
...
HTMLBLOC;
     ...
     */
    public function testSmtpSendSocket()
    {
        if ($this->skipIf(!@fsockopen('localhost', 25), '%s No SMTP server running on localhost')) {
            return;
        }
        ...
        $this->Controller->EmailTest->smtpOptions['timeout'] = 10;
        $socket = new CakeSocket(array_merge(['protocol' => 'smtp'], $this->Controller->EmailTest->smtpOptions));
        $this->Controller->EmailTest->setConnectionSocket($socket);
...
HTMLBLOC;
    ...
    public function testReset()
    {
        ...
        $this->assertIdentical($this->Controller->EmailTest->to, []);
        ...
        $this->assertIdentical($this->Controller->EmailTest->cc, []);
        $this->assertIdentical($this->Controller->EmailTest->bcc, []);
        ...
        $this->assertIdentical($this->Controller->EmailTest->getHeaders(), []);
        $this->assertNull($this->Controller->EmailTest->getBoundary());
        $this->assertIdentical($this->Controller->EmailTest->getMessage(), []);
        $this->assertNull($this->Controller->EmailTest->smtpError);
        $this->assertIdentical($this->Controller->EmailTest->attachments, []);
        $this->assertNull($this->Controller->EmailTest->textMessage);
     ...
     */
    public function testFormatAddressAliases()
    {
     ...
     */
    public function testFormatAddressMultibyte()
    {
