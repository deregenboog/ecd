 ...
 */
App::import('Core', ['View', 'Controller']);
...
class ViewTest extends CakeTestCase
{
    ...
    {
        $this->assertEqual($this->View->viewVars, ['testData' => 'Some test data', 'test2' => 'more data', 'test3' => 'even more data']);
    }
    ...
    public function testAddInlineScripts()
    {
        ...
        $this->View->addScript('prototype.js');
        $this->assertEqual($this->View->__scripts, ['prototype.js']);

        $this->View->addScript('mainEvent', 'Event.observe(window, "load", function() { doSomething(); }, true);');
        $this->assertEqual($this->View->__scripts, ['prototype.js', 'mainEvent' => 'Event.observe(window, "load", function() { doSomething(); }, true);']);
    }
     ...
     */
    public function testElementExtensions()
    {
        $this->View->ext = '.xml';
        $result = $this->View->element('test_element');
    ...
    public function testElementParamsDontOverwriteHelpers()
    {
        ...
        $result = $View->element('type_check', ['form' => 'string'], true);
        $this->assertEqual('string', $result);

        $View->set('form', 'string');
        $result = $View->element('type_check', [], true);
        $this->assertEqual('string', $result);
    }
    ...
    public function testElementCacheHelperNoCache()
    {
        ...
        $View = new View($Controller);
        $empty = [];
        $helpers = $View->_loadHelpers($empty, ['cache']);
        $View->loaded = $helpers;
    ...
    public function testElementCache()
    {
        ...
        $cached = false;
        $result = $View->element($element, ['cache' => '+1 second']);
        ...
        $cached = false;
        $result = $View->element($element, ['cache' => true, 'param' => 'val']);
        ...
        $cached = false;
        $result = $View->element($element, ['cache' => '+1 second', 'other_param' => true, 'anotherParam' => true]);
        ...
        $cached = false;
        $result = $View->element($element, ['cache' => ['time' => '+1 second', 'key' => '/whatever/here']]);
        ...
        $cached = false;
        $result = $View->element($element, ['cache' => ['time' => '+1 second', 'key' => 'whatever_here']]);
    ...
    public function testLoadHelpers()
    {
        ...
        $loaded = [];
        $result = $View->loadHelpers($loaded, ['Html', 'Form', 'Ajax']);
        ...
        $result = $View->loadHelpers($loaded, ['TestPlugin.PluggedHelper']);
    ...
    {
        $this->PostsController->helpers = ['Session', 'Html', 'CallbackMock'];
        $View = new TestView($this->PostsController);
        $loaded = [];
    ...
    {
        $this->PostsController->helpers = ['Session', 'TestAfter', 'Html'];
    ...
    {
        $this->PostsController->helpers = ['Session', 'Html', 'Form', 'Ajax'];
        $View = new TestView($this->PostsController);

        $result = $View->_render($View->getViewFileName('index'), []);
        ...
        $this->PostsController->helpers = ['Html', 'Form', 'Ajax', 'TestPlugin.PluggedHelper'];
        $View = new TestView($this->PostsController);
     ...
     */
    public function testRenderLayoutWithMockCacheHelper()
    {
    ...
    */
    ...
    public function testSet()
    {
        ...
        $this->assertIdentical($View->viewVars, ['somekey' => 'someValue']);
        $this->assertIdentical($View->getVars(), ['somekey']);

        $View->viewVars = [];
        $keys = ['key1', 'key2'];
        $values = ['value1', 'value2'];
        $View->set($keys, $values);
        $this->assertIdentical($View->viewVars, ['key1' => 'value1', 'key2' => 'value2']);
        $this->assertIdentical($View->getVars(), ['key1', 'key2']);
    ...
    public function testEntityReference()
    {
        ...
        $View->model = 'Post';
        $View->field = 'title';
        $this->assertEqual($View->entity(), ['Post', 'title']);

        $View->association = 'Comment';
        $View->field = 'user_id';
        $this->assertEqual($View->entity(), ['Comment', 'user_id']);
