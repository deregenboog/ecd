...
class TextHelperTest extends CakeTestCase
{
    ...
    public function testTruncate()
    {
        ...
        $this->assertIdentical($this->Text->truncate($text1, 15, ['exact' => false]), 'The quick...');
        ...
        $this->assertIdentical($this->Text->truncate($text2, 10, ['exact' => false]), '...');
        ...
        $this->assertIdentical($this->Text->truncate($text5, 6, ['ending' => '']), '0<b>1<');
        $this->assertIdentical($this->Text->truncate($text1, 15, ['html' => true]), 'The quick br...');
        $this->assertIdentical($this->Text->truncate($text1, 15, ['exact' => false, 'html' => true]), 'The quick...');
        $this->assertIdentical($this->Text->truncate($text2, 10, ['html' => true]), 'Heiz&ouml;lr...');
        $this->assertIdentical($this->Text->truncate($text2, 10, ['exact' => false, 'html' => true]), '...');
        $this->assertIdentical($this->Text->truncate($text3, 20, ['html' => true]), '<b>&copy; 2005-2007, Cake...</b>');
        $this->assertIdentical($this->Text->truncate($text4, 15, ['html' => true]), '<img src="mypic.jpg"> This image ...');
        $this->assertIdentical($this->Text->truncate($text4, 45, ['html' => true]), '<img src="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But t...</b>');
        $this->assertIdentical($this->Text->truncate($text4, 90, ['html' => true]), '<img src="mypic.jpg"> This image tag is not XHTML conform!<br><hr/><b>But the following image tag should be conform <img src="mypic.jpg" alt="Me, myself and I" /></b><br />Grea...');
        $this->assertIdentical($this->Text->truncate($text5, 6, ['ending' => '', 'html' => true]), '0<b>1<i>2<span class="myclass">3</span>4<u>5</u></i></b>');
        $this->assertIdentical($this->Text->truncate($text5, 20, ['ending' => '', 'html' => true]), $text5);
        $this->assertIdentical($this->Text->truncate($text6, 57, ['exact' => false, 'html' => true]), "<p><strong>Extra dates have been announced for this year's...</strong></p>");
    ...
    public function testHighlightHtml()
    {
        ...
        $this->assertEqual($this->Text->highlight($text3, ['strong', 'what'], $options), $text3);

        $expected = '<b>What</b> a <b>strong</b> mouse: <img src="what-a-strong-mouse.png" alt="What a strong mouse!" />';
        $this->assertEqual($this->Text->highlight($text4, ['strong', 'what'], $options), $expected);
    }
    ...
    public function testAutoLinkEmails()
    {
        ...
        $result = $this->Text->autoLinkEmails($text, ['class' => 'link']);
        $this->assertPattern('#^'.$expected.'$#', $result);
    }
