...
App::import('Helper', ['Html', 'Form', 'Javascript', 'Ajax']);
/**
...
class TestJavascriptHelper extends JavascriptHelper
{
     ...
     */
    public function codeBlock($parameter)
    {
...
class AjaxHelperTest extends CakeTestCase
{
     ...
     */
    public function testEvalScripts()
    {
        $result = $this->Ajax->link('Test Link', 'http://www.cakephp.org', ['id' => 'link1', 'update' => 'content', 'evalScripts' => false]);
        $expected = [
            'a' => ['id' => 'link1', 'onclick' => ' event.returnValue = false; return false;', 'href' => 'http://www.cakephp.org'],
            'Test Link',
            '/a',
        ...
        $result = $this->Ajax->link('Test Link', 'http://www.cakephp.org', ['id' => 'link1', 'update' => 'content']);
        $expected = [
            'a' => ['id' => 'link1', 'onclick' => ' event.returnValue = false; return false;', 'href' => 'http://www.cakephp.org'],
            'Test Link',
            '/a',
    ...
    {
        $result = $this->Ajax->autoComplete('PostAjaxTest.title', '/posts', ['minChars' => 2]);
        ...
        $result = $this->Ajax->autoComplete('PostAjaxTest.title', '/posts', ['paramName' => 'parameter']);
        $this->assertPattern('/^<input[^<>]+name="data\[PostAjaxTest\]\[title\]"[^<>]+autocomplete="off"[^<>]+\/>/', $result);
        $this->assertPattern('/<div[^<>]+id="PostAjaxTestTitle_autoComplete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<div[^<>]+class="auto_complete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<\/div>\s+<script type="text\/javascript">\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*'.str_replace('/', '\\/', preg_quote('new Ajax.Autocompleter(\'PostAjaxTestTitle\', \'PostAjaxTestTitle_autoComplete\', \'/posts\',')).'/', $result);
        ...
        $result = $this->Ajax->autoComplete('PostAjaxTest.title', '/posts', ['paramName' => 'parameter', 'updateElement' => 'elementUpdated', 'afterUpdateElement' => 'function (input, element) { alert("updated"); }']);
        $this->assertPattern('/^<input[^<>]+name="data\[PostAjaxTest\]\[title\]"[^<>]+autocomplete="off"[^<>]+\/>/', $result);
        $this->assertPattern('/<div[^<>]+id="PostAjaxTestTitle_autoComplete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<div[^<>]+class="auto_complete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<\/div>\s+<script type="text\/javascript">\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*'.str_replace('/', '\\/', preg_quote('new Ajax.Autocompleter(\'PostAjaxTestTitle\', \'PostAjaxTestTitle_autoComplete\', \'/posts\',')).'/', $result);
        ...
        $result = $this->Ajax->autoComplete('PostAjaxTest.title', '/posts', ['callback' => 'function (input, queryString) { alert("requesting"); }']);
        $this->assertPattern('/^<input[^<>]+name="data\[PostAjaxTest\]\[title\]"[^<>]+autocomplete="off"[^<>]+\/>/', $result);
        $this->assertPattern('/<div[^<>]+id="PostAjaxTestTitle_autoComplete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<div[^<>]+class="auto_complete"[^<>]*><\/div>/', $result);
        $this->assertPattern('/<\/div>\s+<script type="text\/javascript">\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*'.str_replace('/', '\\/', preg_quote('new Ajax.Autocompleter(\'PostAjaxTestTitle\', \'PostAjaxTestTitle_autoComplete\', \'/posts\',')).'/', $result);
        ...
        $result = $this->Ajax->autoComplete('PostAjaxText.title', '/post', ['parameters' => "'key=value&key2=value2'"]);
        $this->assertPattern('/{parameters:\'key=value&key2=value2\'}/', $result);

        $result = $this->Ajax->autoComplete('PostAjaxText.title', '/post', ['with' => "'key=value&key2=value2'"]);
        $this->assertPattern('/{parameters:\'key=value&key2=value2\'}/', $result);
    }
    ...
    {
        $result = $this->Ajax->link('Test Link', '/', ['id' => 'link1', 'update' => 'content', 'type' => 'synchronous']);
        $expected = [
            'a' => ['id' => 'link1', 'onclick' => ' event.returnValue = false; return false;', 'href' => '/'],
            'Test Link',
            '/a',
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
    ...
    {
        $result = $this->Ajax->drag('id', ['handle' => 'other_id']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->drag('id', ['onDrag' => 'doDrag', 'onEnd' => 'doEnd']);
     ...
     */
    public function testDroppable()
    {
        $result = $this->Ajax->drop('droppable', ['accept' => 'crap']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->dropRemote('droppable', ['accept' => 'crap'], ['url' => '/posts']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->dropRemote('droppable', ['accept' => ['crap1', 'crap2']], ['url' => '/posts']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->dropRemote('droppable', ['accept' => 'crap'], ['url' => '/posts', 'with' => '{drag_id:element.id,drop_id:dropon.id,event:event.whatever_you_want}']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
    ...
    {
        $result = $this->Ajax->form('showForm', 'post', ['model' => 'Form', 'url' => ['action' => 'showForm', 'controller' => 'forms'], 'update' => 'form_box']);
        $this->assertNoPattern('/model=/', $result);

        $result = $this->Ajax->form('showForm', 'post', ['name' => 'SomeFormName', 'id' => 'MyFormID', 'url' => ['action' => 'showForm', 'controller' => 'forms'], 'update' => 'form_box']);
    ...
    {
        $result = $this->Ajax->sortable('ull', ['constraint' => false, 'ghosting' => true]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->sortable('ull', ['constraint' => 'false', 'ghosting' => 'true', 'update' => 'myId']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->sortable('faqs', ['url' => 'http://www.cakephp.org',
            'update' => 'faqs',
            'tag' => 'tbody',
            'handle' => 'grip',
        ...
        $result = $this->Ajax->sortable('div', ['overlap' => 'foo']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->sortable('div', ['block' => false]);
        $expected = "Sortable.create('div', {});";
        $this->assertEqual($result, $expected);

        $result = $this->Ajax->sortable('div', ['block' => false, 'scroll' => 'someID']);
        $expected = "Sortable.create('div', {scroll:'someID'});";
        $this->assertEqual($result, $expected);

        $result = $this->Ajax->sortable('div', ['block' => false, 'scroll' => 'window']);
        $expected = "Sortable.create('div', {scroll:window});";
        $this->assertEqual($result, $expected);

        $result = $this->Ajax->sortable('div', ['block' => false, 'scroll' => "$('someElement')"]);
     ...
     */
    public function testSubmitWithIndicator()
    {
        $result = $this->Ajax->submit('Add', ['div' => false, 'url' => 'http://www.cakephp.org', 'indicator' => 'loading', 'loading' => 'doSomething()', 'complete' => 'doSomethingElse() ']);
    ...
    public function testLink()
    {
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['confirm' => 'Are you sure & positive?']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+id="link\d+"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+id="link\d+"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'id' => 'myLink']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'id' => 'myLink', 'complete' => 'myComplete();']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+id="myLink"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'id' => 'myLink', 'loading' => 'myLoading();', 'complete' => 'myComplete();']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+id="myLink"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'encoding' => 'utf-8']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        $this->assertPattern('/<script[^<>]+type="text\/javascript"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'success' => 'success();']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        $this->assertPattern('/<script[^<>]+type="text\/javascript"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', 'http://www.cakephp.org/downloads', ['update' => 'myDiv', 'failure' => 'failure();']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        $this->assertPattern('/^<a[^<>]+href="http:\/\/www.cakephp.org\/downloads"[^<>]*>/', $result);
        $this->assertPattern('/^<a[^<>]+onclick="\s*event.returnValue = false;\s*return false;"[^<>]*>/', $result);
        $this->assertPattern('/<script[^<>]+type="text\/javascript"[^<>]*>/', $result);
        ...
        $result = $this->Ajax->link('Ajax Link', '/test', ['complete' => 'test']);
        $this->assertPattern('/^<a[^<>]+>Ajax Link<\/a><script [^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*[^<>]+\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->link(
            'Ajax Link',
            ['controller' => 'posts', 'action' => 'index', '?' => ['one' => '1', 'two' => '2']],
            ['update' => 'myDiv', 'id' => 'myLink']
        );
     ...
     */
    public function testRemoteTimer()
    {
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org']);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'frequency' => 25]);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'complete' => 'complete();']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'complete' => 'complete();', 'create' => 'create();']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'exception' => 'alert(exception);']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'contentType' => 'application/x-www-form-urlencoded']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'method' => 'get', 'encoding' => 'utf-8']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->remoteTimer(['url' => 'http://www.cakephp.org', 'postBody' => 'var1=value1']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new PeriodicalExecuter\(function\(pe\) {.+}, 10\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
     ...
     */
    public function testObserveField()
    {
        $result = $this->Ajax->observeField('field', ['url' => 'http://www.cakephp.org']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        ...
        $result = $this->Ajax->observeField('field', ['url' => 'http://www.cakephp.org', 'frequency' => 15]);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        ...
        $result = $this->Ajax->observeField('field', ['url' => 'http://www.cakephp.org', 'update' => 'divId']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new Form.Element.EventObserver\(\'field\', function\(element, value\) {.+}\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
        ...
        $result = $this->Ajax->observeField('field', ['url' => 'http://www.cakephp.org', 'update' => 'divId', 'with' => 'Form.Element.serialize(\'otherField\')']);
        $this->assertPattern('/^<script[^<>]+type="text\/javascript"[^<>]*>.+<\/script>$/s', $result);
        $this->assertNoPattern('/<script[^<>]+[^type]=[^<>]*>/', $result);
        $this->assertPattern('/^<script[^<>]+>\s*'.str_replace('/', '\\/', preg_quote('//<![CDATA[')).'\s*new Form.Element.EventObserver\(\'field\', function\(element, value\) {.+}\)\s*'.str_replace('/', '\\/', preg_quote('//]]>')).'\s*<\/script>$/', $result);
     ...
     */
    public function testObserveForm()
    {
        $result = $this->Ajax->observeForm('form', ['url' => 'http://www.cakephp.org']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->observeForm('form', ['url' => 'http://www.cakephp.org', 'frequency' => 15]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->observeForm('form', ['url' => 'http://www.cakephp.org', 'update' => 'divId']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->observeForm('form', ['url' => 'http://www.cakephp.org', 'update' => 'divId', 'with' => "Form.serialize('otherForm')"]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
    ...
    public function testSlider()
    {
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['axis' => 'vertical']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['axis' => 'vertical', 'minimum' => 60, 'maximum' => 288, 'alignX' => -28, 'alignY' => -5, 'disabled' => true]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['change' => "alert('changed');"]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['change' => "alert('changed');", 'slide' => "alert('sliding');"]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['values' => [10, 20, 30]]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->slider('sliderId', 'trackId', ['range' => '$R(10, 30)']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
    ...
    public function testRemoteFunction()
    {
        ...
        $result = $this->Ajax->remoteFunction(['update' => 'myDiv', 'confirm' => 'Are you sure?']);
    ...
    public function testDiv()
    {
        ...
        $this->assertTags($result, ['div' => ['id' => 'myDiv']]);
    ...
    public function testEditor()
    {
        ...
        $result = $this->Ajax->editor('myDiv', '/', ['complete' => 'testComplete();']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->editor('myDiv', '/', ['callback' => 'callback();']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->editor('myDiv', '/', ['collection' => [1 => 'first', 2 => 'second']]);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
        ...
        $result = $this->Ajax->editor('myDiv', '/', ['var' => 'myVar']);
        $expected = [
            ['script' => ['type' => 'text/javascript']],
            $this->cDataStart,
