...
class TimeHelperTest extends CakeTestCase
{
    ...
    public function testToQuarter()
    {
        ...
        $this->assertEqual($result, ['2007-01-01', '2007-03-31']);

        $result = $this->Time->toQuarter('2007-5-25', true);
        $this->assertEqual($result, ['2007-04-01', '2007-06-30']);

        $result = $this->Time->toQuarter('2007-8-25', true);
        $this->assertEqual($result, ['2007-07-01', '2007-09-30']);

        $result = $this->Time->toQuarter('2007-12-25', true);
        $this->assertEqual($result, ['2007-10-01', '2007-12-31']);
    }
    ...
    {
        $result = $this->Time->timeAgoInWords(strtotime('+4 months +2 weeks +3 days'), ['end' => '8 years'], true);
        $this->assertEqual($result, '4 months, 2 weeks, 3 days');

        $result = $this->Time->timeAgoInWords(strtotime('+4 months +2 weeks +2 days'), ['end' => '8 years'], true);
        $this->assertEqual($result, '4 months, 2 weeks, 2 days');

        $result = $this->Time->timeAgoInWords(strtotime('+4 months +2 weeks +1 day'), ['end' => '8 years'], true);
        $this->assertEqual($result, '4 months, 2 weeks, 1 day');

        $result = $this->Time->timeAgoInWords(strtotime('+3 months +2 weeks +1 day'), ['end' => '8 years'], true);
        $this->assertEqual($result, '3 months, 2 weeks, 1 day');

        $result = $this->Time->timeAgoInWords(strtotime('+3 months +2 weeks'), ['end' => '8 years'], true);
        $this->assertEqual($result, '3 months, 2 weeks');

        $result = $this->Time->timeAgoInWords(strtotime('+3 months +1 week +6 days'), ['end' => '8 years'], true);
        $this->assertEqual($result, '3 months, 1 week, 6 days');

        $result = $this->Time->timeAgoInWords(strtotime('+2 months +2 weeks +1 day'), ['end' => '8 years'], true);
        $this->assertEqual($result, '2 months, 2 weeks, 1 day');

        $result = $this->Time->timeAgoInWords(strtotime('+2 months +2 weeks'), ['end' => '8 years'], true);
        $this->assertEqual($result, '2 months, 2 weeks');

        $result = $this->Time->timeAgoInWords(strtotime('+2 months +1 week +6 days'), ['end' => '8 years'], true);
        $this->assertEqual($result, '2 months, 1 week, 6 days');

        $result = $this->Time->timeAgoInWords(strtotime('+1 month +1 week +6 days'), ['end' => '8 years'], true);
        ...
        for ($i = 0; $i < 200; ++$i ) {
            ...
            if ($years > 0) {
                ...
                // seconds only
                $relative_date .= ($relative_date ? ', -' : '-').$seconds.' second'.(1 != $seconds ? 's' : '');
            }

            if ('1/1/70' != date('j/n/y', strtotime(str_replace(',', '', $relative_date)))) {
                $result = $this->Time->timeAgoInWords(strtotime(str_replace(',', '', $relative_date)), ['end' => '8 years'], true);
                if ('0 seconds' == $relative_date) {
        ...
        $result = $this->Time->timeAgoInWords(strtotime('-2 years -5 months -2 days'), ['end' => '3 years'], true);
        ...
        $result = $this->Time->timeAgoInWords(strtotime('+2 months +2 days'), ['end' => '1 month']);
        $this->assertEqual($result, 'on '.date('j/n/y', strtotime('+2 months +2 days')));

        $result = $this->Time->timeAgoInWords(strtotime('+2 months +2 days'), ['end' => '3 month']);
        $this->assertPattern('/2 months/', $result);

        $result = $this->Time->timeAgoInWords(strtotime('+2 months +12 days'), ['end' => '3 month']);
        $this->assertPattern('/2 months, 1 week/', $result);

        $result = $this->Time->timeAgoInWords(strtotime('+3 months +5 days'), ['end' => '4 month']);
        ...
        $result = $this->Time->timeAgoInWords(strtotime('+2 months +2 days'), ['end' => '1 month', 'format' => 'Y-m-d']);
        $this->assertEqual($result, 'on '.date('Y-m-d', strtotime('+2 months +2 days')));

        $result = $this->Time->timeAgoInWords(strtotime('-2 months -2 days'), ['end' => '1 month', 'format' => 'Y-m-d']);
        $this->assertEqual($result, 'on '.date('Y-m-d', strtotime('-2 months -2 days')));

        $result = $this->Time->timeAgoInWords(strtotime('-13 months -5 days'), ['end' => '2 years']);
        $this->assertEqual($result, '1 year, 1 month, 5 days ago');

        $fourHours = $this->Time->timeAgoInWords(strtotime('-5 days -2 hours'), ['userOffset' => -4]);
        $result = $this->Time->timeAgoInWords(strtotime('-5 days -2 hours'), ['userOffset' => 4]);
    ...
    {
        App::build([
            'locales' => [TEST_CAKE_CORE_INCLUDE_PATH.'tests'.DS.'test_app'.DS.'locale'.DS],
        ], true);
