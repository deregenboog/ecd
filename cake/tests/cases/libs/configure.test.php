...
class ConfigureTest extends CakeTestCase
{
     ...
     */
    public function testSetErrorReportingLevel()
    {
     ...
     */
    public function testInteractionOfDebugAndLog()
    {
        Configure::write('log', false);
    ...
    {
        App::build(['plugins' => [TEST_CAKE_CORE_INCLUDE_PATH.'tests'.DS.'test_app'.DS.'plugins'.DS]], true);
    ...
    public function testStoreAndLoad()
    {
        ...
        $expected = ['data' => 'value with backslash \, \'singlequote\' and "doublequotes"'];
        ...
        $expected = [
            'data' => ['first' => 'value with backslash \, \'singlequote\' and "doublequotes"', 'second' => 'value2'],
            'data2' => 'value',
        ];
...
class AppImportTest extends CakeTestCase
{
    ...
    public function testBuildWithReset()
    {
        ...
        App::build(['models' => ['/path/to/models/']], true);
    ...
    public function testCore()
    {
        ...
        $this->assertEqual([ROOT.DS.LIBS.'model'.DS], $model);

        $view = App::core('views');
        $this->assertEqual([ROOT.DS.LIBS.'view'.DS], $view);

        $controller = App::core('controllers');
        $this->assertEqual([ROOT.DS.LIBS.'controller'.DS], $controller);
    }
    ...
    public function testListObjects()
    {
        ...
        $notExpected = ['AppModel', 'ModelBehavior', 'ConnectionManager',  'DbAcl', 'Model', 'CakeSchema'];
    ...
    public function testClassLoading()
    {
        ...
        $file = App::import('Core', ['NonExistingPlugin.NonExistingModel.AnotherChild'], false);
        $this->assertFalse($file);
     ...
     *
     * @see http://cakephp.lighthouseapp.com/projects/42648/tickets/410
     */
    public function testImportingHelpersFromAlternatePaths()
    {
        ...
        App::build([
            'helpers' => [TEST_CAKE_CORE_INCLUDE_PATH.'tests'.DS.'test_app'.DS.'views'.DS.'helpers'.DS],
        ]);
        App::build(['vendors' => [TEST_CAKE_CORE_INCLUDE_PATH]]);
        $this->assertFalse(class_exists('BananaHelper'), 'BananaHelper exists, cannot test importing it.');
        App::import('Helper', 'Banana');
    ...
    {
        $file = App::import('File', 'RealFile', false, [], TEST_CAKE_CORE_INCLUDE_PATH.'config'.DS.'config.php');
        $this->assertTrue($file);

        $file = App::import('File', 'NoFile', false, [], TEST_CAKE_CORE_INCLUDE_PATH.'config'.DS.'cake'.DS.'config.php');
        $this->assertFalse($file);
    }
    ...
    {
        $type = ['type' => 'File', 'name' => 'SomeName', 'parent' => false,
                'file' => TEST_CAKE_CORE_INCLUDE_PATH.DS.'config'.DS.'config.php', ];
        $file = App::import($type);
        $this->assertTrue($file);

        $type = ['type' => 'File', 'name' => 'NoFile', 'parent' => false,
    ...
    {
        $file = App::import('File', 'Name', false, [], TEST_CAKE_CORE_INCLUDE_PATH.'config'.DS.'config.php', true);
        $this->assertTrue($file);

        $this->assertTrue(isset($file['Cake.version']));

        $type = ['type' => 'File', 'name' => 'OtherName', 'parent' => false,
    ...
    {
        $file = App::import('File', 'NewName', false, [TEST_CAKE_CORE_INCLUDE_PATH], 'config.php');
        $this->assertTrue($file);

        $file = App::import('File', 'AnotherNewName', false, [LIBS], 'config.php');
        $this->assertFalse($file);
    }
    ...
    {
        $type = ['type' => 'File', 'name' => 'RandomName', 'parent' => false, 'file' => 'config.php', 'search' => [TEST_CAKE_CORE_INCLUDE_PATH]];
        $file = App::import($type);
        $this->assertTrue($file);

        $type = ['type' => 'File', 'name' => 'AnotherRandomName', 'parent' => false, 'file' => 'config.php', 'search' => [LIBS]];
        $file = App::import($type);
        $this->assertFalse($file);
    }
    ...
    public function testMultipleLoading()
    {
        ...
        $load = App::import(['I18n', 'SomeNotFoundClass', 'CakeSocket']);
    ...
    */
    public function testLoadingVendor()
    {
        ...
        ob_start();
        $result = App::import('Vendor', 'TestPlugin.TestPluginAsset', ['ext' => 'css']);
        $text = ob_get_clean();
        $this->assertTrue($result);
        $this->assertEqual($text, 'this is the test plugin asset css file');

        ob_start();
        $result = App::import('Vendor', 'TestAsset', ['ext' => 'css']);
        $text = ob_get_clean();
        $this->assertTrue($result);
        ...
        ob_start();
        $result = App::import('Vendor', 'SomeName', ['file' => 'some.name.php']);
        $text = ob_get_clean();
        $this->assertTrue($result);
