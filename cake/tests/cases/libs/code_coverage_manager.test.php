...
class CodeCoverageManagerTest extends CakeTestCase
{
     ...
     */
    public function testNoTestCaseSupplied()
    {
        if ($this->skipIf(PHP_SAPI == 'cli', 'Is cli, cannot run this test %s')) {
            return;
        }
        $reporter = new CakeHtmlReporter(null, ['group' => false, 'app' => false, 'plugin' => false]);
     ...
     */
    public function testNoTestCaseSuppliedNoErrors()
    {
        if ($this->skipIf(PHP_SAPI == 'cli', 'Is cli, cannot run this test %s')) {
            return;
        }
        ...
        $contents[1] = array_filter($contents[1], [&$this, '_basenameFilter']);
     ...
     */
    public function _basenameFilter($var)
    {
     ...
     */
    public function testGetTestObjectFileNameFromTestCaseFile()
    {
     ...
     */
    public function testOfHtmlDiffReport()
    {
        $manager = &CodeCoverageManager::getInstance();
        $code = <<<PHP
/**
...
PHP;
     ...
     */
    public function testArrayStrrpos()
    {
        $manager = &CodeCoverageManager::getInstance();
     ...
     */
    public function testGetExecutableLines()
    {
        $manager = &CodeCoverageManager::getInstance();
        $code = <<<HTML
			\$manager =& CodeCoverageManager::getInstance();
...
HTML;
     ...
     */
    public function testCalculateCodeCoverage()
    {
        $manager = &CodeCoverageManager::getInstance();
        $data = [
            '25' => [100, 25],
