...
class FolderTest extends CakeTestCase
{
    ...
    public function testZeroAsDirectory()
    {
        ...
        $result = $Folder->read(true, ['.', '..', 'logs', '.svn']);
    ...
    public function testFolderTree()
    {
        ...
        $this->assertIdentical(array_diff($expected[0], $result[0]), []);
        $this->assertIdentical(array_diff($result[0], $expected[0]), []);

        $result = $Folder->tree(TEST_CAKE_CORE_INCLUDE_PATH.'config', false, 'dir');
        $this->assertIdentical(array_diff($expected[0], $result), []);
        $this->assertIdentical(array_diff($result, $expected[0]), []);

        $result = $Folder->tree(TEST_CAKE_CORE_INCLUDE_PATH.'config', false, 'files');
        $this->assertIdentical(array_diff($expected[1], $result), []);
        $this->assertIdentical(array_diff($result, $expected[1]), []);
    }
     ...
     *
     * @see   https://trac.cakephp.org/ticket/6259
     */
