...
class SanitizeTest extends CakeTestCase
{
    ...
    public function testClean()
    {
        ...
        $result = Sanitize::clean($string, ['connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $string = 'test & "quote" \'other\' ;.$ symbol.'."\r".'another line';
        $expected = 'test & '.Sanitize::escape('"quote"', 'test_suite').' '.Sanitize::escape('\'other\'', 'test_suite').' ;.$ symbol.another line';
        $result = Sanitize::clean($string, ['encode' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $string = 'test & "quote" \'other\' ;.$ \\$ symbol.'."\r".'another line';
        $expected = 'test & "quote" \'other\' ;.$ $ symbol.another line';
        $result = Sanitize::clean($string, ['encode' => false, 'escape' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $string = 'test & "quote" \'other\' ;.$ \\$ symbol.'."\r".'another line';
        $expected = 'test & "quote" \'other\' ;.$ \\$ symbol.another line';
        $result = Sanitize::clean($string, ['encode' => false, 'escape' => false, 'dollar' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $string = 'test & "quote" \'other\' ;.$ symbol.'."\r".'another line';
        $expected = 'test & "quote" \'other\' ;.$ symbol.'."\r".'another line';
        $result = Sanitize::clean($string, ['encode' => false, 'escape' => false, 'carriage' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $array = [['test & "quote" \'other\' ;.$ symbol.'."\r".'another line']];
        $expected = [['test &amp; &quot;quote&quot; &#039;other&#039; ;.$ symbol.another line']];
        $result = Sanitize::clean($array, ['connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $array = [['test & "quote" \'other\' ;.$ \\$ symbol.'."\r".'another line']];
        $expected = [['test & "quote" \'other\' ;.$ $ symbol.another line']];
        $result = Sanitize::clean($array, ['encode' => false, 'escape' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $array = [['test odd Ä spacesé']];
        $expected = [['test odd &Auml; spaces&eacute;']];
        $result = Sanitize::clean($array, ['odd_spaces' => false, 'escape' => false, 'connection' => 'test_suite']);
        $this->assertEqual($result, $expected);

        $array = [['\\$', ['key' => 'test & "quote" \'other\' ;.$ \\$ symbol.'."\r".'another line']]];
        $expected = [['$', ['key' => 'test & "quote" \'other\' ;.$ $ symbol.another line']]];
        $result = Sanitize::clean($array, ['encode' => false, 'escape' => false]);
        $this->assertEqual($result, $expected);
...
HTML;
    ...
    public function testFormatColumns()
    {
        ...
        $this->DataTest = new SanitizeDataTest(['alias' => 'DataTest']);
        $data = ['DataTest' => [
                        'id' => 'z',
                        'count' => '12a',
                        'float' => '2.31456',
        ...
        $this->Article = new SanitizeArticle(['alias' => 'Article']);
        $data = ['Article' => [
            'id' => 'ZB',
            'user_id' => '12',
