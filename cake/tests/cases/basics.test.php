...
class BasicsTest extends CakeTestCase
{
    ...
    {
        $this->skipIf(!function_exists('ini_get') || '1' === ini_get('safe_mode'), '%s safe mode is on');
    ...
    public function testA()
    {
        ...
        $this->assertEqual(['this', 'that', 'bar'], $result);
    }
    ...
    public function testParams()
    {
        ...
        $this->assertEqual(params(['weekend']), ['weekend']);

        $nested = [['weekend']];
        $this->assertEqual(params($nested), ['weekend']);

        $multiple = [['weekend'], 'jean-luc', 'godard'];
    ...
    {
        $this->skipIf('1' === ini_get('magic_quotes_sybase'), '%s magic_quotes_sybase is on');
    ...
    {
        $this->skipUnless('1' === ini_get('magic_quotes_sybase'), '%s magic_quotes_sybase is off');
    ...
    public function testIfe()
    {
        ...
        $this->assertEqual(ife(['t' => 'est'], 'a', 'b'), 'a');
    ...
    public function testPluginSplit()
    {
        ...
        $this->assertEqual($result, ['Something', 'else']);

        $result = pluginSplit('Something.else.more.dots');
        $this->assertEqual($result, ['Something', 'else.more.dots']);

        $result = pluginSplit('Somethingelse');
        $this->assertEqual($result, [null, 'Somethingelse']);
        ...
        $this->assertEqual($result, ['Blog', 'Post']);
