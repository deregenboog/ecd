...
class ControllerTaskTest extends CakeTestCase
{
    ...
    public function testListAll()
    {
        ...
        $this->Task->expectAt(1, 'out', ['1. Articles']);
        $this->Task->expectAt(2, 'out', ['2. ArticlesTags']);
        $this->Task->expectAt(3, 'out', ['3. Comments']);
        $this->Task->expectAt(4, 'out', ['4. Tags']);

        $expected = ['Articles', 'ArticlesTags', 'Comments', 'Tags'];
        $result = $this->Task->listAll('test_suite');
        $this->assertEqual($result, $expected);

        $this->Task->expectAt(6, 'out', ['1. Articles']);
        $this->Task->expectAt(7, 'out', ['2. ArticlesTags']);
        $this->Task->expectAt(8, 'out', ['4. Comments']);
        $this->Task->expectAt(9, 'out', ['5. Tags']);

        $this->Task->interactive = false;
        $result = $this->Task->listAll();

        $expected = ['articles', 'articles_tags', 'comments', 'tags'];
        $this->assertEqual($result, $expected);
    }
    ...
    public function testDoHelpers()
    {
        ...
        $result = $this->Task->doHelpers();
        $expected = ['Javascript', 'Ajax', 'CustomOne'];
        $this->assertEqual($result, $expected);
    ...
    public function testDoComponents()
    {
        ...
        $result = $this->Task->doComponents();
        $expected = ['RequestHandler', 'Security'];
        $this->assertEqual($result, $expected);
    ...
    public function testConfirmController()
    {
        ...
        $this->Task->expectAt(2, 'out', ["Controller Name:\n\t$controller"]);
        $this->Task->expectAt(3, 'out', ["Helpers:\n\tAjax, Time"]);
        $this->Task->expectAt(4, 'out', ["Components:\n\tAcl, Auth"]);
    ...
    public function testBakeActionsUsingSessions()
    {
        ...
        $this->assertTrue(false !== strpos($result, 'function index() {'));
        $this->assertTrue(false !== strpos($result, '$this->Article->recursive = 0;'));
        $this->assertTrue(false !== strpos($result, "\$this->set('articles', \$this->paginate());"));

        $this->assertTrue(false !== strpos($result, 'function view($id = null)'));
        $this->assertTrue(false !== strpos($result, "\$this->Session->setFlash(__('Invalid article', true));"));
        $this->assertTrue(false !== strpos($result, "\$this->set('article', \$this->Article->read(null, \$id)"));

        $this->assertTrue(false !== strpos($result, 'function add()'));
        $this->assertTrue(false !== strpos($result, 'if (!empty($this->data))'));
        $this->assertTrue(false !== strpos($result, 'if ($this->Article->save($this->data))'));
        $this->assertTrue(false !== strpos($result, "\$this->Session->setFlash(__('The article has been saved', true));"));

        $this->assertTrue(false !== strpos($result, 'function edit($id = null)'));
        $this->assertTrue(false !== strpos($result, "\$this->Session->setFlash(__('The article could not be saved. Please, try again.', true));"));

        $this->assertTrue(false !== strpos($result, 'function delete($id = null)'));
        $this->assertTrue(false !== strpos($result, 'if ($this->Article->delete($id))'));
        $this->assertTrue(false !== strpos($result, "\$this->Session->setFlash(__('Article deleted', true));"));

        $result = $this->Task->bakeActions('Articles', 'admin_', true);

        $this->assertTrue(false !== strpos($result, 'function admin_index() {'));
        $this->assertTrue(false !== strpos($result, 'function admin_add()'));
        $this->assertTrue(false !== strpos($result, 'function admin_view($id = null)'));
        $this->assertTrue(false !== strpos($result, 'function admin_edit($id = null)'));
        $this->assertTrue(false !== strpos($result, 'function admin_delete($id = null)'));
    }
    ...
    public function testBakeActionsWithNoSessions()
    {
        ...
        $this->assertTrue(false !== strpos($result, 'function view($id = null)'));
        $this->assertTrue(false !== strpos($result, "\$this->flash(__('Invalid article', true), array('action' => 'index'))"));
        $this->assertTrue(false !== strpos($result, "\$this->set('article', \$this->Article->read(null, \$id)"));

        $this->assertTrue(false !== strpos($result, 'function add()'));
        $this->assertTrue(false !== strpos($result, 'if (!empty($this->data))'));
        $this->assertTrue(false !== strpos($result, 'if ($this->Article->save($this->data))'));
        $this->assertTrue(false !== strpos($result, "\$this->flash(__('The article has been saved.', true), array('action' => 'index'))"));

        $this->assertTrue(false !== strpos($result, 'function edit($id = null)'));
        $this->assertTrue(false !== strpos($result, "\$this->Article->Tag->find('list')"));
        $this->assertTrue(false !== strpos($result, "\$this->set(compact('tags'))"));

        $this->assertTrue(false !== strpos($result, 'function delete($id = null)'));
        $this->assertTrue(false !== strpos($result, 'if ($this->Article->delete($id))'));
        $this->assertTrue(false !== strpos($result, "\$this->flash(__('Article deleted', true), array('action' => 'index'))"));
    }
    ...
    public function testBakeTest()
    {
        ...
        $this->Task->Test->expectOnce('bake', ['Controller', 'Articles']);
    ...
    public function testInteractive()
    {
        ...
        $this->Task->expectAt(0, 'createFile', [$filename, new PatternExpectation('/class ArticlesController/')]);
    }
    ...
    public function testExecuteWithControllerNameVariations()
    {
        ...
        $this->Task->args = ['Articles'];
        $filename = '/my/path/articles_controller.php';
        $this->Task->expectAt(4, 'createFile', [
            $filename, new PatternExpectation('/class ArticlesController/'),
        ]);
        $this->Task->execute();
        $this->Task->execute();
    ...
    public function testExecuteWithPublicParam()
    {
        ...
        $this->Task->connection = 'test_suite';
        $this->Task->path = '/my/path/';
        $this->Task->args = ['Articles', 'public'];
    ...
    public function testExecuteWithControllerAndBoth()
    {
        ...
        $this->Task->connection = 'test_suite';
        $this->Task->path = '/my/path/';
        $this->Task->args = ['Articles', 'public', 'admin'];
