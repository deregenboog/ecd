<div class="awbzHoofdaannemers index">
	<h2><?php __('Awbz Hoofdaannemers');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('klant_id');?></th>
			<th><?php echo $this->Paginator->sort('begindatum');?></th>
			<th><?php echo $this->Paginator->sort('einddatum');?></th>
			<th><?php echo $this->Paginator->sort('hoofdaannemer_id');?></th>
			<th><?php echo $this->Paginator->sort('created');?></th>
			<th><?php echo $this->Paginator->sort('modified');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($awbzHoofdaannemers as $awbzHoofdaannemer):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $awbzHoofdaannemer['AwbzHoofdaannemer']['id']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($awbzHoofdaannemer['Klant']['name'], array('controller' => 'klanten', 'action' => 'view', $awbzHoofdaannemer['Klant']['id'])); ?>
		</td>
		<td><?php echo $awbzHoofdaannemer['AwbzHoofdaannemer']['begindatum']; ?>&nbsp;</td>
		<td><?php echo $awbzHoofdaannemer['AwbzHoofdaannemer']['einddatum']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($awbzHoofdaannemer['Hoofdaannemer']['naam'], array('controller' => 'hoofdaannemers', 'action' => 'view', $awbzHoofdaannemer['Hoofdaannemer']['id'])); ?>
		</td>
		<td><?php echo $awbzHoofdaannemer['AwbzHoofdaannemer']['created']; ?>&nbsp;</td>
		<td><?php echo $awbzHoofdaannemer['AwbzHoofdaannemer']['modified']; ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $awbzHoofdaannemer['AwbzHoofdaannemer']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $awbzHoofdaannemer['AwbzHoofdaannemer']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $awbzHoofdaannemer['AwbzHoofdaannemer']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $awbzHoofdaannemer['AwbzHoofdaannemer']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true),
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
	 |	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true).' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Awbz Hoofdaannemer', true), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Klanten', true), array('controller' => 'klanten', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Klant', true), array('controller' => 'klanten', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Hoofdaannemers', true), array('controller' => 'hoofdaannemers', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Hoofdaannemer', true), array('controller' => 'hoofdaannemers', 'action' => 'add')); ?> </li>
	</ul>
</div>
