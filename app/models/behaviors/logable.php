...
class LogableBehavior extends ModelBehavior
{
            ...
            if (isset($data['id'])) {
                if ('Attachment' != $r_model) {
                    ...
                    $attachments = $Model->Attachment->find('all', $modelID,
                        ['conditions' => ['created' > $date, 'foreign_key' => $modelID]]
                    );
                    $res = [];
                    foreach ($attachments as &$file) {
                        ...
                        $res[] = ['Log' => [
                            ...
                            'foreign_key' => $attach['id'],
                             'action' => 'add',
                             'user_id' => null,
                             'change' => 'id () => ('.$attach['id'].'), basename () => ('.$attach['basename'].'), size () => ('.$attach['size'].')',
                        ]];
                    }
                }
                ...
                foreach ($data as $key => $child) {
                    if (is_array($child) && isset($child['id'])) {
                        ...
                        $res = $Model->quickFindLog($r_model, $id, $cond, $order);
    ...
    public function findUserActions(&$Model, $user_id, $params = [])
    {
        ...
        if (isset($this->user)
             ...
             && $this->user[$this->UserModel->alias][$this->UserModel->primaryKey] == $user_id
