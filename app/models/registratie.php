...
class Registratie extends AppModel {
	...
	function getRegistratiesConditions($locatie_id = null, $type = 'active'){
		$this->Behaviors->attach('Containable');
		
		$and = array('locatie_id' => $locatie_id);
		if($type == 'active') $and['buiten'] = null;
	...
	function getActiveRegistraties(&$regular_klanten, &$gebruikers, $locatie_id, $sort = null ){
	...
	function getRecentlyUnregistered($locatie_id, $previous_days = 0, &$active_registr = array(), &$gebruiker_registr = array() ) {
	...
	function automaticCheckOut( $conditions, $now = null ) {
	...
	function countActiveRegistraties(&$registraties){
	...
	function get_timelimit($locationId, $previous_days = 0){
	...
	function registratieCheckOut($registratie_id = null, $now = null){
	...
	function removeKlantFromAllQueueLists(&$registratie) {
	...
	function delKlantFromQueueList($registratie_id = null, $fieldname, &$registraties, &$registratie_data){
	...
	function updateQueueList($action, $fieldname, $registratie_id = null, $locatie_id){
	...
	function delKlantFromShowerList($registratie_id = null, &$registraties, &$registratie_data){
	...
	function updateShowerList($action, $registratie_id = null, $locatie_id){
	...
	function addRegistratie($klant_id = null, $locatie_id = null){
	...
	function set_checkboxes_from_previous_registration(&$previous_registration){
		if(empty($previous_registration['Registratie'])) return;
	...
	function setMessages(&$data){
		...
		foreach($data as $key => $val){
			...
			if($unseenOpm === 1) $unseenOpm = '1 opmerking';
			elseif($unseenOpm > 1) $unseenOpm = $unseenOpm . ' opmerkingen';
			else $unseenOpm = 'geen';  
	...
	function _buildQuery($locatie_id, &$sort_string){
		$q = 'SELECT * FROM('.
		...
		if(strlen($sort_string)) $q .= ' ORDER BY '.$sort_string;
		$q .= ';';
		return $q;
	}
	
	function getLocatieName($locatie_id){
