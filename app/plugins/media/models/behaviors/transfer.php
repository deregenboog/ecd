...
class TransferBehavior extends ModelBehavior
{
     ...
     *  :uuid:                  An uuid generated by Str::uuid()
     *  :day:                   The current day
    ...
    public function setup(&$Model, $config = null)
    {
        ...
        if (isset($Model->validate['file'])) {
            $default = ['allowEmpty' => true, 'required' => false, 'last' => true];
     ...
     */
    public function prepare(&$Model, $file = null)
    {
        ...
        $Folder = new Folder($destination['dirname'], $createDirectory, 0777);
     ...
     */
    public function _source(&$Model, $data)
    {
        if (TransferValidation::fileUpload($data)) {
            return array_merge($this->info($Model, $data), ['error' => $data['error']]);
        } elseif (MediaValidation::file($data)) {
            return $this->info($Model, $data);
        } elseif (TransferValidation::url($data, ['scheme' => 'http'])) {
            return $this->info($Model, $data);
        }
     ...
     * @return mixed Array with parsed results on success, false on error
     */
    public function _temporary(&$Model, $data)
    {
        if (TransferValidation::fileUpload($data)
     ...
     * @return mixed Array with parsed results on success, false on error
     */
    public function _destination(&$Model, $data)
    {
    ...
    public function perform(&$Model)
    {
        ...
        $chain = implode('>>', [$source['type'], $temporary['type'], $destination['type']]);

        if ('http-url-remote' === $source['type']) {
            if (!class_exists('HttpSocket')) {
                App::import('Core', 'HttpSocket');
            }
            $Socket = new HttpSocket(['timeout' => 5]);
            $Socket->request(['method' => 'GET', 'uri' => $source['file']]);
     ...
     * @return mixed
     */
    public function getLastTransferredFile(&$Model)
    {
        extract($this->runtime[$Model->alias], EXTR_SKIP);
    ...
    public function checkLocation(&$Model, $field, $allow = true)
    {
        ...
        foreach (['source', 'temporary', 'destination'] as $type) {
            if ('temporary' == $type && empty($$type)) {
                continue;
            }
            if ('source' == $type && 'file-upload-remote' == ${$type}['type']) {
                continue;
            }
    ...
    public function checkPermission(&$Model, $field, $match = true)
    {
        ...
        foreach (['source', 'temporary'] as $type) {
            if ('temporary' == $type && empty($$type)) {
                continue;
            }
    ...
    public function checkPixels(&$Model, $field, $max = false)
    {
        ...
        foreach (['source', 'temporary'] as $type) { /* pixels value is optional */
            if (('temporary' == $type && empty($$type)) || !isset(${$type}['pixels'])) {
                continue;
            }
    ...
    public function checkExtension(&$Model, $field, $deny = false, $allow = true)
    {
        ...
        foreach (['source', 'temporary', 'destination'] as $type) {
            if (('temporary' == $type && empty($$type)) || !isset(${$type}['extension'])) {
                continue;
            }
     ...
     * @return mixed
     */
    public function info(&$Model, $resource, $what = null)
    {
        extract($this->settings[$Model->alias], EXTR_SKIP);
        ...
        if (TransferValidation::url($resource, ['scheme' => 'http'])) {
            ...
            $Socket = new HttpSocket(['timeout' => 5]);
            $Socket->request(['method' => 'HEAD', 'uri' => $resource['file']]);
            ...
            $resource = array_merge(
                ...
                [
                    'file' => $resource,
                    'host' => 'localhost',
                    'mimeType' => MimeType::guessType($resource, ['paranoid' => !$trustClient]),
            ]);
     ...
     */
    public function _alternativeFile($file, $tries = 100)
    {
        $extension = null;
     ...
     */
    public function _addMarker(&$Model, $marker, $replacement = null, $slugify = false)
    {
     ...
     * @return string
     */
    public function _replaceMarker(&$Model, $subject)
    {
        if (is_array($subject)) {
        ...
        $subject = Str::insert($subject, $markers, [
            'before' => ':', 'after' => ':',
            'escape' => '#',
            'clean' => [
                'before' => ':', 'after' => ':',
        ...
        if (false !== strpos($subject, 'unknown_marker')) {
