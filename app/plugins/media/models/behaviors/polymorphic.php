...
class PolymorphicBehavior extends ModelBehavior
{
    ...
    {
        $this->settings[$model->name] = am(['classField' => 'class', 'foreignKey' => 'foreign_id'], $config);
    }
    ...
    public function afterFind(&$model, $results, $primary = false)
    {
        ...
        if ($primary && isset($results[0][$model->alias][$classField]) && $model->recursive > 0) {
            foreach ($results as $key => $result) {
                ...
                if ($class && $foreignId) {
                    ...
                    if (!isset($model->$class)) {
                        $model->bindModel(['belongsTo' => [
                            $class => [
                                'conditions' => [$model->alias.'.'.$classField => $class],
                                'foreignKey' => $foreignKey,
                            ],
                        ]]);
                    }
                    // Applied fix suggested in http://groups.google.com/group/cake-php/msg/3ecbdc13ca1e3a64
                    $associated = $model->$class->find([$class.'.id' => $foreignId], null, null, -1);
            ...
            if ($class && $foreignId) {
                ...
                $associated = $model->$class->find([$class.'.id' => $foreignId], ['id', $model->$class->displayField], null, -1);
                $associated[$class]['display_field'] = $associated[$class][$model->$class->displayField];
