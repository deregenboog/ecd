...
class MimeTypeTest extends CakeTestCase
{
    ...
    {
        MimeType::config('magic', ['engine' => false]);
        MimeType::config('glob', ['engine' => false]);
    ...
    public function testGuessTypeFileinfoShippedGlob()
    {
    ...
    public function testGuessTypeParanoid()
    {
        ...
        MimeType::config('magic', ['engine' => 'fileinfo']);
        MimeType::config('glob', ['engine' => 'core', 'file' => dirname(dirname(dirname(dirname(__FILE__)))).DS.'vendors'.DS.'mime_glob.db']);

        $file = $this->TestData->getFile(['image-png.png' => TMP.'image-png.jpg']);
        $this->assertEqual(MimeType::guessType($file, ['paranoid' => true]), 'image/png');
        $this->assertEqual(MimeType::guessType($file, ['paranoid' => false]), 'image/jpeg');
    }
}
