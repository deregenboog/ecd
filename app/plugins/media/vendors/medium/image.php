...
class ImageMedium extends Medium
{
     ...
     * @var array
     */
    public $adapters = ['BasicImage', 'Imagick', 'Gd', 'ImagickShell'];
    ...
    public function crop($width, $height)
    {
        ...
        return $this->Adapters->dispatchMethod($this, 'crop', [$left, $top, $width, $height]);
    }
    ...
    public function zoomCrop($width, $height, $gravity = 'center')
    {
        ...
        return $this->Adapters->dispatchMethod(
            $this,
            'cropAndResize',
            [$zoomLeft, $zoomTop, $zoomWidth, $zoomHeight, $width, $height]
        );
    }
    ...
    public function fitCrop($width, $height, $gravity = 'center')
    {
        ...
        $this->Adapters->dispatchMethod($this, 'resize', [$resizeWidth, $resizeHeight]);
    ...
    public function compress($value = 1.5)
    {
        ...
        return $this->Adapters->dispatchMethod($this, 'compress', [floatval($value)]);
    }
    ...
    {
        return (int) ($this->width() * $this->height() / 1000000);
    }
     ...
     * @param int $width
     * @param int $height
     * @param int $set    Either "ratio" or "maximum" Maximum replaces sizes execeeding limits
     *                    with medium's corresponding size
     *
     * @return array An array containing width and height
     */
    public function _normalizeDimensions($width, $height, $set = 'ratio')
    {
        if ($width > $this->width()) {
            $width = null;
        }
     ...
     */
    public function _boxify($width, $height, $gravity = 'center')
    {
        if ($width > $this->width()) {
            $left = 0;
        } else {
