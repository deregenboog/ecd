...
class MimeGlob extends Object
{
     ...
     * @param bool   $reverse Enable/disable reverse searching
     *
     ...
     *
     * @see http://httpd.apache.org/docs/2.2/en/mod/mod_mime.html
    ...
    public function __read($db)
    {
        ...
        if ('Array' === $format) {
        ...
        } elseif ('Apache Module mod_mime' === $format) {
            $File = new File($db);
            $File->open('rb');
            ...
            while (!feof($File->handle)) {
                ...
                $this->register(['mime_type' => array_shift($line), 'pattern' => $line]);
            }
        } else {
     ...
     */
    public function __test($name, $items, $caseSensitive = true)
    {
     ...
     */
    public function __testReverse($mimeType, $items)
    {
        $results = [];
