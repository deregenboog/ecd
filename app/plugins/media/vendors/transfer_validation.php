...
class TransferValidation extends MediaValidation
{
    ...
    {
        if (self::fileUpload($check)
         || self::uploadedFile($check) /* This must appear above file */
         || MediaValidation::file($check)
         || self::url($check)) {
            return !self::blank($check);
        }
    ...
    public static function blank($check)
    {
        ...
        if (self::fileUpload($check) && UPLOAD_ERR_NO_FILE == $check['error']) {
            return true;
        }
     ...
     */
    public static function url($check, $options = [])
    {
        if (!is_string($check)) {
            return false;
        }
