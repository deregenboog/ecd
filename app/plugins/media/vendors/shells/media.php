 ...
 */
App::import('Core', ['ConnectionManager', 'Folder']);
...
class MediaShell extends Shell
{
     ...
     * @var string
     */
    public $tasks = ['Sync', 'Make', 'Collect'];
    /**
     * Verbose mode.
     *
     * @var bool
    ...
    public function protect()
    {
        ...
        if ('n' == $this->in('Do you want to create it now?', 'y,n', 'n')) {
            return false;
        }
     ...
     */
    public function heading($string, $width = null, $character = '=')
    {
        if (is_string($width)) {
            $character = $width;
            $width = null;
     ...
     * @param mixed  $width
     */
    public function hr($character = '-', $width = null)
    {
        $this->out(str_repeat($character, null === $width ? $this->width : $width));
    }
     ...
     * @param mixed $message
     *
     * @see /usr/lib/portage/bin/isolated-functions.sh
     */
    ...
    {
        $this->out(0 === $status ? __('Quitting.', true) : __('Aborting.', true));
        parent::_stop($status);
    }
}
