...
class MimeMagic extends Object
{
    ...
    public function analyze($file, $options = [])
    {
        ...
        $default = ['minPriority' => 0, 'maxPriority' => 100];
     ...
     * @param int   $indent   The nesting depth of the item
     * @param int   $priority A value between 0 and 100.
     *                        Low numbers should be used for more generic types and higher values for specific subtypes.
     *
     * @return bool True if item has successfully been registered, false if not
     */
     ...
     *
     * @see http://httpd.apache.org/docs/2.2/en/mod/mod_mime_magic.html
     * @see http://standards.freedesktop.org/shared-mime-info-spec/shared-mime-info-spec-0.13.html
     */
    public function __read($db)
    {
        $format = self::format($db);
        ...
        if ('Array' === $format) {
        ...
        } elseif ('Freedesktop Shared MIME-info Database' === $format) {
            ...
            while (!feof($File->handle)) {
                ...
                if (!isset($chars)) {
                    $chars = [0 => $File->read(1), 1 => $File->read(1)];
                } else {
        ...
        } elseif ('Apache Module mod_mime_magic' === $format) {
     ...
     */
    public function __test($file, $items)
    {
        $File = new File($file);
     ...
     * @return mixed A string containing the MIME type of the file or false if no pattern matched
     */
    public function __testRecursive(&$File, $item)
    {
        if (isset($item['mask'])) {
        ...
        if (false !== strpos($File->read($item['value_length'] + $item['range_length']), $item['value'])) {
            if (isset($item['and'])) {
     ...
     * @param bool  $binary   Whether the value is a binary value or not
     * @param bool  $unEscape If set to true and value is not binary strips slashes from string values
     *
     * @return mixed On success the formatted binary value or the input value
     */
    public function __formatValue($value, $type, $toBinary = false, $unEscape = true)
    {
        if (!$toBinary) {
            switch ($type) {
                case 2:
