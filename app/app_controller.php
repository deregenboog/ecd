...
class AppController extends Controller {
	 ...
	 * The list of controllers accessible per group are
	...
	public function _isControllerAuthorized($controller) {
		...
		foreach ( $this->userGroups as $gid => $g_name ) {
			...
			if (in_array ( $controller, $enable [$gid] ) || in_array ( "*", $enable [$gid] )) {
	 ...
	 * A position can be specified, if the parameter
	...
	/**
	 * forAdminOnly If you are not admin, redirect.
	 * For quick access handling
	 * in actions.
	 *
	 * @access public
	 * @return void
	 ...
	 *
	 * @param unknown_type $medewerker_ids
	 *        	can be an array of medewerkers with should be in the array even when inactive
	 */
	...
	/**
	 * flash Wrapper for Session-flash.
	 * See flashError, it is used more.
	 *
	 * @param mixed $msg
	 * @access public
	 * @return void
	...
	public function beforeFilter() {
		...
		$this->AuthExt->loginAction = array (
				'controller' => 'medewerkers',
				'action' => 'login'
		);
		$this->AuthExt->logoutRedirect = array (
				'controller' => 'medewerkers',
				'action' => 'login'
		);
		// this is to allow everyone to see the static pages
		$this->AuthExt->allowedActions = array (
				'display'
		);
		...
		if (isset ( $auth ['Medewerker'] )) {
			...
			if (sizeof ( $this->uses ) && property_exists ( $this->modelClass, 'Behaviors' ) && $this->{$this->modelClass}->Behaviors->attached ( 'Logable' )) {
				$activeUser = array (
						'Medewerker' => array (
		...
		if ($s_user || array_key_exists ( GROUP_ADMIN, $this->userGroups ) || array_key_exists ( GROUP_DEVELOP, $this->userGroups )) {
			$is_admin = true;
		} else {
			$is_admin = false;
		}
	...
	public function beforeRender() {
		...
		if (! empty ( $this->{$model} ) && is_array ( $this->{$model}->hasAndBelongsToMany )) {
	...
	{
		return Registry::getInstance()->getManager();
	}
	...
	protected function createForm($type, $data = null, array $options = array())
	{
		...
		$formFactory = Forms::createFormFactoryBuilder()
			->addExtension(new DoctrineOrmExtension(Registry::getInstance()))
