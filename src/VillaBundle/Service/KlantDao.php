...
//            $sql = $builder->getQuery()->getSQL();
    ...
    public function countResultatenPerLocatie($page = null, FilterInterface $filter = null)
    {
        $builder = $this->repository->createQueryBuilder('klant')
            /**
             * SELECT DISTINCT (klanten.id), villa_dossier_statussen.class, villa_resultaten.naam FROM klanten LEFT JOIN verslagen ON verslagen.klant_id = klanten.id
            LEFT JOIN villa_dossier_statussen ON villa_dossier_statussen.id = klanten.huidigeMwStatus_id
            LEFT JOIN villa_afsluiting_resultaat ON villa_afsluiting_resultaat.afsluiting_id = villa_dossier_statussen.id
            INNER JOIN villa_resultaten ON villa_resultaten.id = villa_afsluiting_resultaat.resultaat_id
            WHERE verslagen.id IS NOT NULL
            AND villa_dossier_statussen.class IN ('Afsluiting')
             */
        ;
