...
//            $sql = $builder->getQuery()->getSQL();
    ...
    public function countResultatenPerLocatie($page = null, FilterInterface $filter = null)
    {
        $builder = $this->repository->createQueryBuilder('klant')
            /**
             * SELECT DISTINCT (klanten.id), clip_dossier_statussen.class, clip_resultaten.naam FROM klanten LEFT JOIN verslagen ON verslagen.klant_id = klanten.id
            LEFT JOIN clip_dossier_statussen ON clip_dossier_statussen.id = klanten.huidigeMwStatus_id
            LEFT JOIN clip_afsluiting_resultaat ON clip_afsluiting_resultaat.afsluiting_id = clip_dossier_statussen.id
            INNER JOIN clip_resultaten ON clip_resultaten.id = clip_afsluiting_resultaat.resultaat_id
            WHERE verslagen.id IS NOT NULL
            AND clip_dossier_statussen.class IN ('Afsluiting')
             */
        ;
