...
class VragenPerStadsdeel extends AbstractReport
{
    protected $title = 'Vragen per stadsdeel';
    ...
    {
        $vragen = $this->vraagDao->countByStadsdeel($this->startDate, $this->endDate);
        array_walk($vragen, function (&$item) {
            $item['kolom'] = 'Vragen';
        });

        $contactmomenten = $this->contactmomentDao->countByStadsdeel($this->startDate, $this->endDate);
