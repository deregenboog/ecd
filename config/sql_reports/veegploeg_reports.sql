...
-- FIELDS: 0.naam - Locatie;  0.Aantal unieke bezoekers - Aantal unieke bezoekers
-- ARRAY
-- !DISABLE
-- SUMMARY
...
(SELECT
    ...
    IFNULL((SELECT COUNT(distinct klant_id) FROM `verslagen` `v` WHERE `v`.`locatie_id` = `tl`.`id`  AND v.`datum` between :from and :until AND v.klant_id IN (SELECT DISTINCT klant_id FROM tmp_visitors_veegploeg)), '-') AS 'Aantal unieke bezoekers'
FROM `locaties` `tl`
ORDER BY `naam`)
UNION
(SELECT
